...
\subsection{Permutation equivalence}

\begin{definition}
	contenuto...
\end{definition}

\begin{theorem}[Characterization of $\equiv_p$]
	contenuto...
\end{theorem}
\begin{proof}
	contenuto...
...
\begin{proof}
	...
	\Cref{prop:pbpoad} we can now deduce the existence of the dotted $t\colon L_{j-1}\to K_{j-1}$, from which the thesis  follows. \qedhere
	\end{enumerate}
\end{proof}
...
\begin{proof}
	...
	We can also notice that,  by \Cref{lem:impo}, $\sigma(i)=\tau(i)$ for every $i\in  [0, l_1-1]$. In particular, this implies that the image  $[l_1, l_1+l_2-1]$ through $\sigma$ is again the interval $[l_1, l_1+l_2-1]$ and that $\varrho$ is well-defined.  Moreover, if $l_1\neq 0$, then $l_1-1$ is the maximum of $E_{\sigma,\tau}$. 
...
On the one hand, computing we get:
		\begin{align*}
	q'_2\circ \psi \circ \iota_{2, G_{2,0}}\circ \alpha_2&=q \alpha_2\\&=\xi_\sigma \circ \iota_{G_{l_1+l_1}} \circ \omega_2\\&=\iota'_{G'_{l_1+l_2}}\circ \omega'_2\\&=q_2\circ \iota'_{2, G'_{2, l_2}}\circ \omega'_{2}
\end{align*}

		\begin{align*}
			q_2\circ \psi \circ \iota_{2, G_{2,l_2}}\circ \omega_2&=\xi_\sigma\circ q_2\circ \iota_{2, G_{2,l_2}}\circ \omega_2\\&=\xi_\sigma \circ \iota_{G_{l_1+l_1}} \circ \omega_2\\&=\iota'_{G'_{l_1+l_2}}\circ \omega'_2\\&=q_2\circ \iota'_{2, G'_{2, l_2}}\circ \omega'_{2}
		\end{align*}
	But $q_2$ is mono and then  $\psi \circ \iota_{2, G_{2,l_2}}\circ \omega_2=\iota'_{2, G'_{\varrho(i)}}\circ \omega'_{2}$ as wanted. 
	...
	\begin{align*}
		q_2\circ \psi \circ \iota_{2, G_{2,i+1}}\circ h_{2,i}&=\xi_\sigma\circ q_2\circ \iota_{2, G_{2,i+1}}\circ h_{2,i}\\&=\xi_\sigma \circ \iota_{G_{i+l_1+1}} \circ h_{i+l_1}\\&=\iota'_{G'_{\sigma(i+l_1)+1}}\circ h'_{\sigma(i+l_1)}\\&=q_2\circ \iota'_{2, G'_{\varrho(i)+1}}\circ h'_{2, \varrho(i)}	
	\end{align*}
	
		
	For every 	
		
		Let now $i$ be an index different from $0$ and $\sigma^{-1}(l_1+1)$, then, using \Cref{rem:dett} and the previous point of this lemma we have:
		\begin{align*}	q_2\circ \psi \circ \iota_{2, G_{2,i}}\circ m_{2,i}&=\xi_\sigma\circ q_2\circ \iota_{2, G_{2,i}}\circ m_{2,i}\\&=\xi_\sigma \circ \iota_{G_{i+l_1}} \circ m_{i+l_1}\\&=\iota'_{G'_{\sigma(i+l_1)}}\circ m'_{\sigma(i+l_1)}\\&=q_2\circ \iota'_{2, G'_{\varrho(i)}}\circ m'_{2, \varrho(i)}
		\end{align*}
	 
	Using the fact that $q_2$ belongs to $\mathcal{M}$ we conclude that 
	\[\psi \circ \iota_{2, G_{2,i+1}}\circ h_{2,i}=\iota'_{2, G'_{\varrho(i)+1}}\circ h'_{2, \varrho(i)}\]
	
		We are left with two cases. First of all, we can notice thatIn this case we have:
		\[G_{i}=\pi(G_{2,0}) \quad G'_{i}=\pi(G'_{2,0}) \quad m_{l_1}= \alpha^{-1}_2 \circ m_{2,0} \quad m'_{l_1}= (\alpha^{-1}_2)^{-1} \circ m'_{2,0} \]
		
		Let's compute:
		\[q_2\circ \psi \circ \iota_{2}\]
...
We can sum up the results of this section recasting them in the language of permutation equivalence.
