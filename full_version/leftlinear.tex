...
\begin{definition}[Concrete  and decorated switch] \label{def:switch}
	Let $\X$ be an $\mathcal{M}$-adhesive category 
	and $(\X, \R)$ a left-linear rewriting system. 
	Let also
	...
	A \emph{concrete switch} of $\der{D}$ along $(i_0,i_1)$ is
	...
	If a concrete switch of $\dder{D}_0$ and $\dder{D}_1$ exists we say that they are \emph{switchable}.
...
Let $\der{E}=\{\dder{E}_i\}_{i=0}^1$ be a concrete switch for $\dder{D}_0$
...
Moreover, $\der{D}$ uses the same rule of $\der{E}$ in reverse order, thus it is a concrete switch for $\der{E}$ along $(i_0', i_1')$.

Similarly, if $(\der{E}, \alpha, \omega)$ is a decorated switch for $(\der{D}, \alpha, \omega)$ along $(i_0, i_1)$, then the latter is a decorated switch for the former along $(i'_0, i'_1)$.
\end{remark}
...
\begin{lemma}\label{lem:switchtoperm}
	...
	\begin{enumerate}
		...
		\[\kappa \circ g_0 = \iota_{\der{E}, G_0}\circ f_0 \qquad \kappa\circ h_0= \iota_{\der{E}, R_0}\]
\item there exists an isomorphism $\phi\colon \tpro{D}\to \tpro{E}$ such that
\[\phi \circ \iota_{\der{D}, G_0}=\iota_{\der{E}, G_0} \qquad \phi \circ \iota_{\der{D}, G_1}=\kappa \qquad \phi \circ \iota_{\der{D}, G_2}=\iota_{\der{E}, G_2}\]
		\item $2$-cycle $\tau_{0,1}$ is a consistent permutation between $(\der{D}, \alpha, \omega)$ and $(\der{D}, \alpha, \omega)$.
	\end{enumerate}
\end{lemma}
...
\begin{lemma}[Uniqueness of concrete switches]
