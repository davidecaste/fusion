...
\begin{proof}
	...
	Now the thesis follows applying \Cref{lem:radj} to the pair of squares on the right. Since $f_0$ and $p_0$ are element of $\mathcal{M}$, then $f_{\der{E},0}\circ z_0$ is in $\mathcal{M}$ too and so $z_0$ is in $\mathcal{M}$ by \Cref{prop:deco}.	
	\end{proof}
	

\begin{example}\label{ex:abs2}
Let $\der{D}$ be the derivation of \Cref{ex:abs} and consider the pre-switch $\der{F}$. Then we can build the diagram below
...
By \Cref{thm:switch_uni} and \Cref{ex:abs2} we know that being a switch is not a property invariant under  abstraction equivalence. Nonetheless we  can still prove an analog to \Cref{lem:abs}.

\begin{proposition}\label{prop:switchabs}
	Let $\der{D}=\{\dder{D}_i\}_{i=0}^1$ be the following derivation:
\[\xymatrix@R=20pt@C=20pt{L_0 \ar[d]_{m_0}&& K_0
	\ar[d]_{k_0}\ar@{>->}[ll]_{l_0} \ar[r]^{r_0} & R_0
	\ar@{>}@/^.25cm/[drrr]|(.32)\hole_(.4){i_0}
...
By \Cref{lem:abs} and our hypotheses, we know that $\der{E}$ is a pre-switch, thus by \Cref{lem:cose} we can consider the two diagrams below, whose right halves are pullbacks.
...
\end{split}\]
	...
	\end{align*}
	We can conclude by \Cref{lem:equi}.
	
	\item By the second point of \Cref{lem:abs} we know that $\der{G}$ is a pre-switch along $(i_0, i_1)$. Suppose that $\der{G}$ is given by the derivation below.
	\[
	\xymatrix@R=20pt@C=20pt{
		...
		& & {G_2}  }
	\]
	Again by \Cref{lem:cose} we know that there are diagrams as the following ones, whose common right half is a pullback.
...
By \Cref{rem:abscol}, we know that there exists an isomorphism $\Psi\colon \tpro{E}\to \tpro{G}$ such that
...
\end{align*}
	We conclude again by \Cref{lem:equi}.\qedhere 
\end{enumerate}
 \end{proof}


\begin{corollary}\label{cor:swdeco}
	contenuto...
\end{corollary}
...
\begin{definition}[Concrete switching equivalence]
