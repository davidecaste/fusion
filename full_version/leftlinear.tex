...
\begin{lemma}[Uniqueness of switches]
	...
	$\dder{D}_1$. Then there is an abstraction equivalence between $\der{E}$ and $\der{F}$, whose first component is $\id{G_0}$.
\end{lemma}
...
\begin{remark}\label{rem:deco} 
	...
	We can prove some other properties of
		...
		first and third rectangles are pullbacks too.
...
\begin{proof}
	...
	\[\xymatrix{L_1 \ar[d]_{f_0 \circ i_1}&& K_1
		...
		H_1 && \ar@{>->}[ll]^{a_1} Q_1 \ar[rr]_{b_1}&& G_2}\] It is immediate
...
\begin{corollary}\label{cor:strongip}
	...
	\begin{enumerate}
		...
		then the squares below are pushouts.
		\[\xymatrix{K_1 \ar[d]_{e_0} \ar[r]^{{r_1}}& R_1 \ar[d]^{i'_0} & K_1 \ar@{>->}[r]^{l_1} \ar[d]_{e_1}& L_1 \ar[d]^{i'_1} \\
			T\ar[r]_{t_1} & D'_1 &T \ar@{>->}[r]_{t_0}&D'_0}\]
...
\begin{proof}
	\begin{enumerate}
		\item Let us consider the derivation below, constructed in the proof of \Cref{prChurch}. 
		\[\xymatrix{L_1 \ar[d]_{f_0 \circ i_1}&& K_1
			\ar[d]_{q_0\circ u_1}\ar@{>->}[ll]_{l_1} \ar[r]^{r_1} & R_1
			\ar@/^.35cm/[drrr]|(.31)\hole_(.4){j_0} \ar[dr]|(.3)\hole_{a_1\circ
				j_0} && L_0 \ar@/_.35cm/[dlll]^(.4){j_1} \ar[dl]|(.3)\hole^{b_0\circ
		...
		By \Cref{thm:switch_uni} we know that there exists three isomorphisms $\phi_{Q_0}\colon Q_0\to D'_0$, $\phi_{Q_1}\colon Q_1\to D'_1$ and $\phi_{H_1}\colon H_1\to G'_1$ fitting in the diagrams below.
		...
		\[\begin{split}
		g'_0\circ \phi_{Q_0}\circ q_0&=\phi_{H_1}\circ b_0\circ q_0\\&=\phi_{H_1}\circ a_1\circ q_1\\&= f'_1\circ \phi_{Q_1}\circ q_1
		\end{split} \qquad  \begin{split}b_0\circ \phi^{-1}_{Q_0} \circ t_0&=\phi^{-1}_{H_1} \circ g'_0\circ t_0\\&=\phi^{-1}_{H_1} \circ f'_1\circ t_1\\&= a_1\circ \phi^{-1}_{Q_1}\circ t_1
		\end{split}\]
		therefore, by the universal property of pullbacks we get the dotted arrows $v\colon P\to T$ and $w\colon T\to P$. To see that they are one the inverse of the other it is then enough to compute.
		\[\begin{split}
		...
		\begin{align*}
	f'_1\circ \circ  \phi_{Q_1}\circ j_0&=\phi_{H_1}\circ a_1\circ j_0\\&= h'_0\\&=f'_1\circ i'_0
		\end{align*}
		thus, since $f'_1$ is mono, we have $\phi_{Q_1}\circ j_0=i'_0$.  The thesis now follows immediately from the diagram below, since the left square is a pushout by construction.		
		\[\xymatrix{K_1  \ar@/^.4cm/[rr]^{e_0}\ar[r]_{u_1} \ar[d]_{r_1}& P \ar[r]_{v} \ar[d]_{q_1} & T \ar[d]^{t} \\ R_1 \ar@/_.4cm/[rr]_{i'_0} \ar[r]^{j_0}& Q_1 \ar[r]^{\phi_{Q_1}} & D'_1}\]
...
We can now turn to prove the fundamental property that we need to rely upon:  any decorated switch induces a consistent permutation.

\begin{lemma}\label{lem:switchtoperm}
	Let $(\der{D}, \alpha, \omega)$ be a decorated derivations of length $2$ and suppose that $(\der{E}, \alpha, \omega)$ is a decorated switch for it along the independence pair $(i_0, i_1)$. Then the following are true:
	\begin{enumerate}
		\item there exists an arrow $\kappa\colon G_1\to \tpro{E}$ such that 
...
\begin{proof}
	...
	\begin{enumerate}
		\item  We can start computing to get:
		\begin{align*}
		...
		\end{align*}
		Thus we get the wanted arrow $\kappa\colon G_1\to \tpro{E}$.
