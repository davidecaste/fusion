...
With these tools available, we are ready to provide a way to construct binary suprema in $\sub{M}{\X}{X}$.
 ...
 \begin{theorem}[\cite{garner2012axioms,castelnovo2023thesis,johnstone2007quasitoposes}]label{prop:un}Let $\X$ be a strict $\mathcal{M}$-adhesive category with pullbacks. If $\mathcal{M}$ contains all split monomorphisms  then for every onject $X$, $(\sub{M}{X}{X}, \leq)$ has binary suprema.
 \end{theorem}
...
\begin{example}\label{ex:sem}
	\todo{nei grafi semplici questa cosa non funziona}
\end{example}
