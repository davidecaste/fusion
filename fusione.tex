...
\newcommand{\Deltamin}{\Delta^{\hspace{-2pt}\begin{matrix}
			\smallmin\\
...
\begin{definition}
	Let $(\X, \R)$ be a DPO-rewrityng system, with $\X$ $\mathcal{M}$-adhesive. The \emph{DPO-derivation graph} of $(\X, \R)$ is the (large)  directed graph $\gpo$ having as vertices the objects of $\X$ and in which an edge between $G$ and $H$ is a direct derivation $\dder{D}\colon G\Mapsto H$.	A \emph{derivation} $\der{D}$ between two objects $G$ and $H$ is a path between them in $\gpo$.
\end{definition}
...
\begin{definition}Let $(\X, \R)$ be a DPO-rewriting system,  an \emph{abstraction equivalence} between two derivations $\der{D}$ and $\der{D'}$ with the same length and such that $r(\der{D})=r(\der{D}')$, is a family of isomorphisms $\{\phi_X\}_{X\in \Deltamin(\der{D})}$ such that, for every $i\in [0, \lgh(\der{D})]$ the following diagram commutes
	\[\xymatrix@C=40pt{G'_i&D'_i \ar[r]^{g'_i} \ar[l]_{f'}&G'_{i+1}\\  L_i \ar[u]^{n'_i} \ar[d]_{n_i}& K_i \ar[u]^{k'_i} \ar[d]_{k_i} \ar[r]^{r_i} \ar[l]_{l_i} &R\ar[u]^{h'_i} \ar[d]_{h_i}\\G_i \ar@/_.45cm/[uu]_(.35){\phi_{G_i}}|\hole&D_i\ar[l]^{f_i}\ar@/_.45cm/[uu]_(.35){\phi_{D_i}}|\hole \ar[r]_{g}&G_{i+1}\ar@/_.45cm/[uu]_{\phi_{H_i}}}\]
...
Let $(\der{D}, \alpha, \omega)$ be a decorated derivation between $G$ and $H$ and  $(\der{D}',\alpha', \omega')$ be one between $G'$ and $H'$. Suppose, moreover that $G'$ and $H$ are isomorphic, so that $\pi(G')=\pi(H)$. Then the following hold true.
\begin{enumerate}
	\item If $\der{D}$ is the empty derivation, then
	\[(\der{D}, \alpha, \omega)\cdot (\der{D}',\alpha', \omega')\equiv_a (\der{D}',\alpha', \omega')\]
	\item  If $\der{D}'$ is the empty derivation, then
	\[(\der{D}, \alpha, \omega)\cdot (\der{D}',\alpha', \omega')\equiv_a (\der{D},\alpha, \omega)\]
\end{enumerate}
\end{proposition}
...
\begin{enumerate}
	\item 
	By \Cref{def:conc}, we have that 
	\[(\der{D}, \alpha, \omega)\cdot (\der{D}',\alpha', \omega')= (\der{D}', \alpha'\circ \omega^{-1}\circ \alpha, \omega')\] 
	
	For every $X\in \Deltamin(\der{D'})$ we can define
	\[\phi_{X}:=\begin{cases}
		\alpha' \circ \alpha \circ \omega \circ (\alpha')^{-1} & X=G'\\
		\id{X} & \text{otherwise}
	\end{cases} \]
	Now, we can notice that $\phi_{G'_0}$ is the composition $\alpha'\circ (\alpha'\circ \omega^{-1}\circ \alpha)^{-1}$, giving us the triangle

\[\xymatrix@C=15pt{&\pi(G') \ar[dr]^{\alpha'} \ar[dl]_{\alpha'\circ \omega^{-1}\circ \alpha}\\ G'\ar[rr]_{\phi_{G'}} && G'} \]
To conclude the proof it is now enough to show the commutativity of 
	\[\xymatrix@C=40pt{G'&D'_0 \ar[r]^{g'_0} \ar[l]_{f'_0}&G'_{1}\\  L'_0 \ar[u]^{n'_0} \ar[d]_{n'_0}& K'_0 \ar[u]^{k'_0} \ar[d]_{k'_0} \ar[r]^{r'_0} \ar[l]_{l'_0} &R'_0\ar[u]^{h'_0} \ar[d]_{h'_0}\\G' \ar@/_.45cm/[uu]_(.35){\phi_{G'}}|\hole&D'_0\ar[l]^{f'_0}\ar@/_.45cm/[uu]_(.35){\id{D'_0}}|\hole \ar[r]_{g'_0}&G'_{1}\ar@/_.45cm/[uu]_{\id{G'_1}}}\]
	
	\item \qedhere 
\end{enumerate}
\end{proof}

\begin{remark}\label{rem:ident}Consider a decorated derivation $(\der{D}, \alpha, \omega)$ between $G$ and $H$ and another one $(\der{D}',\alpha', \omega')$ between $G'$ and $H'$. Suppose, moreover that $\pi(G')=\pi(H)$. By \Cref{rem:empty}, if $\der{D}$ is the empty derivation, so that $G=H$, then 
	\[(\der{D}, \alpha, \omega)\cdot (\der{D}',\alpha', \omega')= ((\omega \circ (\alpha')^{-1}) *\der{D}', \alpha, \omega')\]

Furthermore, we can notice that
\[(\der{D}', \alpha', \omega')\equiv_a ((\omega \circ (\alpha')^{-1}) *\der{D}', \alpha, \omega')\]	
Indeed, for every $X'\in \Deltamin(\der{D'})$ we can define
\[\phi_{X'}:=\begin{cases}
	(\alpha')^{-1} & X'=G'\\
	\id{X'} & \text{otherwise}
\end{cases} \]
...
Similarly, if $\lgh(\der{D}')=0$, implying $G'=H'$, we get
		\[(\der{D}, \alpha, \omega)\cdot (\der{D}',\alpha', \omega')=(\der{D}*\omega^{-1}, \alpha, \omega')\]

We can notice, moreover, that 
\[(\der{D}', \alpha', \omega')\equiv_a ((\omega \circ (\alpha')^{-1}) *\der{D}', \alpha, \omega') \qquad  (\der{D}, \alpha, \omega) \equiv_a (\der{D}*\omega^{-1}, \alpha, \omega')\] 	
Indeed,  for every $X'\in \Deltamin(\der{D'})$ and $X\in \Deltamin(\der{D})$ we can define
\[\phi_{X'}:=\begin{cases}
	(\alpha')^{-1} & X'=G'_{0}\\
	\id{X'} & \text{otherwise}
\end{cases} \qquad \phi_{X}:=\begin{cases}
\id{X} & \text{otherwise}\\
\omega^{-1} & X=H\\
\end{cases}\]


first of all notice that the triangles below commute.
\[\xymatrix@C=15pt{&\pi(G'_0) \ar[dr]^{\alpha'} \ar[dl]_{\alpha}&&& \pi(G) \ar[dr]^{\omega'} \ar[dl]_{\omega}\\ G\ar[rr]_{\phi} && G' &G' \ar[rr]_{\phi} && G' } \]


Furthermore, for every $X\in $ $i\in [0, \lgh(\der{D})]$ 
...
The thesis now follows at once.
 ...
 To fix the notation, suppose that $\der{D}$, $\der{D}'$, $\der{E}$ and $\der{E}'$ are given by
	...
	where $\dder{D}_i$, $\dder{D}'_i$, $\dder{E}_i$ and $\dder{E}'_i$ are given, respectively, by the following four diagrams. We can also notice that the rule applied by $\dder{D}_i$ must coincide with the one applpied in $\dder{D}'_i$ and that the same holds for $\dder{E}_i$ and $\dder{E}'_i$.
...
To show that $\{\psi_X\}_{\Deltamin(\der{D}*\omega^{-1}\cdot \beta*\der{E})}$ it is now enough to prove the commutativity of the diagrams
...
Given DPO-rewriting system $(\X, \R)$,  we have already noted in \Cref{rem:func} that a derivation $\der{D}$  determines a diagram $\Delta(\der{D})$ in $\X$. We can then wonder if such a diagram has a colimit. Clealry if $\der{D}$ is the empty derivation $G$ then a colimit for $\Delta(\der{D})$ is simply the object $G$. More generally, we have the following result.
...
\begin{lemma}\label{lem:colim}
	...
	\begin{enumerate}
		\item For every derivation $\der{D}=$ of length $n$ and for every $j\in[0,n-1]$, the outer boundary of following diagram is a pushout,
		\[\xymatrix@C=30pt{D_{j} \ar[r]^{g_j}\ar@{>->}[d]_{f_j}& G_{j+1} \ar[r]^-{\hat{\iota}_{G_{j+1}}} \ar@{>->}@/^.3cm/[ddl]^{\iota'_{G_{j+1}}} & \lpro \der{\hat{D}}\rpro \ar[dd]^{p_2} \\ G_j \ar[d]_{\iota'_{G_j}}\\ \lpro \der{D}' \rpro \ar[rr]_{p_1}  &&\tpro{D} }\] 
		where $\der{D}':=\{\dder{D}_i\}_{i=0}^{j-1}$, $\der{\hat{D}}:=\{\dder{D}_i\}_{i=j+1}^n$ and the two arrows $p_1\colon \lpro\der{D}' \rpro\to \tpro{D}$, $p_2\colon \lpro \der{\hat{D}}\rpro \to \tpro{D}$ are induced by the cocones $(\tpro{D}, \{\iota_{X}\}_{X\in \Delta(\der{D}')})$ and $(\tpro{D}, \{\iota_{X}\}_{X\in \Delta(\der{D}')})$, respectively.
		\iffalse 
	...
	\[\xymatrix{L \ar[d]_{m}& K \ar[d]^{k}\ar@{>->}[l]_{l} \ar[r]^{r} & R \ar[d]^{h} \\G & \ar@{>->}[l]^{f} D \ar[r]_{g}& H }\]	
		Then the colimiting cocone $(\tpro{D}, \{\iota_X\}_{X\in \Delta(\der{D})})$ exists, moreover the following square is an $\mathcal{M}$-pushout;
...
\begin{proof}\begin{enumerate}
		...
		\end{align*}
		Hence, there exists a unique $c\colon P\to C$ such that $c\circ q= c_G$ and $c\circ p =c_H$. But these identities are enough to deduce that $(P, \{j_X\}_{X\in \Delta(\der{D})})$  is colimiting.
		...
		\begin{itemize}
			...
			\item Let $\der{D}$ be $\{\dder{D}_i\}_{i=0}^n$ and suppose that $n\geq 1$. Let also $\der{D}'$ be $\{\dder{D}_i\}^{n-1}_{i=0}$ and $\rho_n=(l_n, r_n)$ be the rule applied in $\dder{D}_n$. The pushout of $l_n$ is the arrow $f_n\colon D_n\to G_n$ is in $\mathcal{M}$ and, by inductive hypothesis, $\iota_{G_{n}}\colon G_{n}\to \lpro \der{D}'\rpro$ is in $\mathcal{M}$ too. Thus, we can consider the diagram below, having a pushout as its lower half.
...
\item Let $a\colon \lpro \der{\hat{D}}\rpro\to C$ , $b\colon \lpro \der{D}' \rpro\to C$ be two arrows fitting in the diagram 
	...
	\end{cases}\]
	Thus we get a unique arrow $c\colon \tpro{D}\to C$ such that 
	\[c\circ \iota_X = c_X\]
	...
	To see that such $c$ is unique, suppose that $c'\colon \tpro{D}\to C$ is given such that
...
\begin{corollary}\label{cor:ele}
	...
	then  there exist arrow $c\colon T\to D_j $  such that \[f_j\circ c = a\qquad \iota_{D_j}\circ c =\iota_{G_k}\circ b\]
	\end{corollary}
...
Let $\sigma\colon [0,n]\to [0,n]$ be a consistent permutation between $(\der{D},\alpha, \omega)$ and $(\der{D}', \alpha', \omega')$, then $\sigma^{-1}$ is a consistent permutation between $(\der{D}', \alpha', \omega')$ and $(\der{D},\alpha, \omega)$: indeed, it is enough to consider, as mediating isomorphism, the inverse $\xi^{-1}_\sigma$ of $\xi_\sigma$.
\end{remark}
...
Now, notice that, given a consistent permutation $\sigma$, $\xi_\sigma\colon \tpro{D}\to \lpro \der{D}'\rpro$, we already know, by \Cref{rem:coproj} that
\[\xi_\sigma \circ \iota_{L_i}=\iota'_{L_{\sigma(i)}} \quad \xi_\sigma \circ \iota_{K_i}=\iota'_{K_{\sigma(i)}} \quad \xi_\sigma \circ \iota_{R_i}=\iota'_{R_{\sigma(i)}} \]
...
\begin{corollary} Given a consistent permutation $\sigma$ between  $(\der{D}, \alpha, \omega)$ and $(\der{D}', \alpha', \omega')$, the mediating isomorphism $\xi_\sigma\colon \tpro{D}\to \lpro \der{D}'\rpro$ is unique.
\end{corollary}
...
\begin{proof}\begin{enumerate}
	...
	From \Cref{cor:ele}, we can conclude that there exists $c\colon L_0\to D_0$ such that $f_0\circ c=m_0$. We thus have the solid part of the commutative diagram below.
		...
		\item  Let $k$ be $\sigma^{-1}(\sigma'(j-1))$ and notice that $\rho_{j-1}=\rho_k$. By definition of $j$, $\sigma_{|[0,j-1]}=\sigma'_{|[0,j-1]}$, thus the second point of \Cref{prop:unique} yields the diagram
			...
			The existence of the dotted $t\colon L_{j-1}\to K_{j-1}$ follows from  \Cref{prop:pbpoad} and we can conclude. \qed
	\end{enumerate}
\end{proof}
...
Let $(i_1,i_2)$ be a good independence pair between the derivation $\dder{D}\colon G\Mapsto H$ and $\dder{D}'\colon H\Mapsto T$.  Let also $\der{\hat{D}}$ be the derivation consisting only in $S_{i_1,i_2}(\dder{D})$. We have a diagram 
...
\begin{proposition}\label{prop:fil}Let $(\X, \R)$ be a left-linear DPO-rewriting system with $\X$ an $\mathcal{M}$-adhesive category. Then every filler between two direct derivations $\dder{D}\colon G\Mapsto H$ and $\dder{D}'\colon H\Mapsto T$ is a filler also for $S_{u,u'}(\dder{D'})\colon G\Mapsto J$ and $S_{u,u'}(\dder{D})\colon J\Mapsto H$. In particular, if $\dder{D}\updownarrow \dder{D}'$, then $S_{u,u'}(\dder{D}')\updownarrow S_{u,u'}(\dder{D})$.
\end{proposition}
...
\begin{definition}[Very tameness]
	\todo{very tame}
\end{definition}
...
\subsubsection{Graphical rewriting systems}
...
\section{Full proof of \Cref{thm:crit}}
