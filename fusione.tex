...
Let $\X$ be a category, a \emph{separating family},  is a class $\mathcal{S}$ of objects such that, for every pair of arrows $f, g:X\rightrightarrows Y$, if \[f\circ s = g\circ s\] for every $s:S\to X$ with domain $S\in \mathcal{S}$, then $f=g$. 

Given a separating family $\mathcal{S}$, we will say that a morphism $f:X\to Y$,  is an \emph{$\mathcal{S}$-surjection} if for every $s:S\to Y$ with domain in $\mathcal{S}$  there exists $s':S\to X$ such that $f\circ s'= s$.
\end{definition}

We can list some examples of separating families.
...
\end{example}
\begin{proposition}
Let $f:X\to Y$ be a morphism in a category $\X$ with a separating family $\mathcal{S}$, then every $\mathcal{S}$-surjection is an epimorphism.
\end{proposition}
\begin{proof}
	Let $f:X\to Y$ be an $\mathcal{S}$-surjection, and suppose that there exist two arrows $g, h:Y\rightrightarrows Z$ such that 
	\[g\circ f = h\circ f\]
Since $f$ is an $\mathcal{S}$-surjection, for every $S\in \mathcal{S}$ and arrow $s:S\to Y$ there exists $s':S\to X$ such that $f\circ s'=s$, so that
\begin{align*}
	g\circ s &= g\circ f \circ s'\\&=h\circ f \circ s'\\&= h\circ s
\end{align*} 
	The thesis now follows since $\mathcal{S}$ is separating.	 \qed 
\end{proof}

We are now ready to prove the central result of this section. 

\begin{lemma} Let $(\X,R)$ ba left-linear DPO-rewriting system and suppose that $\X$ has a separator $\mathcal{S}$. Consider two consistent permutation $\sigma,\sigma':[0,n]\rightrightarrows [0,n]$ between $(\der{D}, \alpha, \omega)$ and $(\der{D}', \alpha', \omega')$. Suppose that $\sigma\neq \sigma'$ and let $j$ be the minimum index such that $\sigma(j)\neq \sigma'(j)$. Let also $r(\der{D})$ be $\{\rho_i\}_{i=0}^n$. Then the following hold true:
	\begin{enumerate}
...
Let $(\X,R)$ ba left-linear DPO-rewriting system and suppose that every rule in $R$ is consuming. Given two non-empty derivations $\der{D}=\{\dder{D}_{i}\}_{i=0}^n$ and $\der{D}'=\{\dder{D}'_{i}\}_{i=0}^n$, if $\X$ has a separator $\mathcal{S}$, then there exists at most one consistent permutation between them.
\end{corollary}
...
\begin{proof} Consider two consistent permutations $\sigma$ and $\sigma': n+1 \rightrightarrows n+1$ and let $\xi, \xi':\tpro{D}\rightrightarrows \lpro \der{D}' \rpro$ be the associated isomorphisms.	\[\xymatrix@C=8pt@R=20pt{ &&&&&R_{i}\ar@/^1.4cm/[dddrrr]^{h'_{\sigma(i)}} \ar@/_1.7cm/[dddlll]_{h'_{\sigma'(i)}}|(.65)\hole|(.83)\hole\ar[ddd]|(.35)\hole^{h_i}|(.805)\hole\\&&&&K_i \ar@/_1.7cm/[dddlll]_{k'_{\sigma'(i)}}|(.67)\hole\ar[dl]_{l_i} \ar[ur]^{r}\ar@/^1.4cm/[dddrrr]^{k'_{\sigma(i)}} \ar[ddd]|(.35)\hole_(.7){k_i}\\&&&L_i\ar@/_1.7cm/[dddlll]_{m'_{\sigma'(i)}}\ar@/^1.4cm/[dddrrr]^(.7){m'_{\sigma(i)}} \ar[ddd]_{m_i}\\&&G'_{\sigma'(i)+1}\ar@/^.5cm/[dddl]^{\iota_{G'_{\sigma'(i)+1}}}&&&G_{i+1}  \ar@/^.5cm/[dddl]^{\iota_{G_{i+1}}}&&& G'_{\sigma(i)+1}\ar@/^.5cm/[dddl]^{\iota_{G'_{\sigma(i)+1}}}\\ & D'_{\sigma'(i)}\ar[ur]^{g'_{\sigma'(i)}}\ar[dl]_(.35){f'_{\sigma'(i)}}\ar[dd]^{\iota_{D'_{\sigma'(i)}}}&&&D_i \ar[dd]^{\iota_{D_i}}\ar[ur]^{g_i}  \ar[dl]_{f_i}&&& D'_\sigma(i) \ar[dd]^{\iota_{D'_\sigma(i)}} \ar[ur]^{g'_{\sigma(i)}}\ar[dl]_(.4){f'_{\sigma(i)}}\\ G'_{\sigma'(i)}\ar@/_.3cm/[dr]_(.35){\iota_{G_{\sigma'(i)}}}&&&G_i \ar@/_.3cm/[dr]_(.35){\iota_{G_i}} &&& G'_{\sigma(i)}\ar@/_.3cm/[dr]_(.35){\iota_{G'_{\sigma(i)}}}\\&\lpro \der{D}'\rpro &&& \tpro{D} \ar[rrr]_{\xi} \ar[lll]^{\xi'}&&& \lpro \der{D}' \rpro}\]
	
Suppose that there exists $j\in n+1$ be such that $\sigma(j)\neq \sigma'(j)$.  Let $k$ be $\sigma^{-1}(\sigma'(i))$, then, in particular $j\neq k$. By \Cref{rem:coproj} we know that
\begin{align*}
\iota_{L_{k}}=\xi^{-1} \circ \iota'_{L_{\sigma(k)}}
\end{align*}We have two cases.
	\begin{itemize}
		\item $i< \sigma^{-1}(\sigma'(i))$.
		\item $\sigma^{-1}(\sigma'(i))<i$.\qed 
	\end{itemize} 
\end{proof} 
	
\begin{proof}  whithout loss of generality we can suppose that $\sigma'(i)<\sigma(i)$. Using consistency, we get a diagram 
