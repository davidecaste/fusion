...
\subsubsection{Final pullback complements}
...
\begin{proof}\begin{enumerate}
	...
	\end{enumerate}
	\end{proof} 
	In particular, we will use the first point of the previous lemma to deduce the following result.
	
	
	\begin{corollary}\label{cor:cond} Let $f\colon A\to C$, $g\colon B\to C$ and $h\colon C\to D$ be arrows in a category $\X$, let also $l_0\colon A'\to A$, $l_1\colon B'\to B$ be two monomorphisms. Suppose moreover, that the following squares are pullbacks:
