...
\begin{remark}A remark on the uniqueness of filler is due. Let $\der{D}:\Mapsto H$ and $\der{D}':G\Mapsto T$ be two direct derivations such that $\der{D}\updownarrow_! \der{D}'$. The uniqueness of the filler between them must be interpreted only up to a choice of a pullback square 
	\[\xymatrix{P \ar[r]^{p} \ar[d]_{p'}& D\ar[d]^{g} \\ D' \ar[r]_{f'} & H}\]
\end{remark}
...
\begin{remark}\label{rem:unic} Given an independence pair $(i_1, i_2)$ between $\dder{D}$ and $\dder{D}'$, there is at most one filler  such that $(u,u')$ such that 
	\[\xymatrix{K \ar[r]^{r} \ar[d]_{u}& R \ar[d]^{i_1} &K' \ar[r]^{l'} \ar[d]_{u'}& L'\ar[d]^{i_2} \\P \ar[r]_{p'}& D' & P \ar[r]_{p} & D}\]
...
\begin{proposition}\label{prop:filip}
Let $(\X, \R)$ be a tame left-linear DPO-rewriting system, then, given $\dder{D}:G\Mapsto H$ and $\dder{D}':H\Mapsto T$, the following are equivalent:
\begin{enumerate}
	\item $\dder{D}$ and $\dder{D}'$ are sequentially independent;
	\item $\dder{D}\updownarrow_! \dder{D}'$.
\end{enumerate}
\end{proposition}
\begin{proof} $(1\Rightarrow 2)$ Let $(i_1, i_2)$ be the unique independence pair between $\dder{D}$ and $\dder{D}$'. Take a pullback square \[\xymatrix{P \ar[r]^{p} \ar[d]_{p'}& D\ar[d]^{g} \\ D' \ar[r]_{f'} & H}\]
by tameness we know that there exist a filler $(u,u')$ made by arrows $u:K\to P$ and $u':K'\to P$ fitting in the two pushout squares below. 
	\[\xymatrix{K \ar[r]^{r} \ar[d]_{u}& R \ar[d]^{i_1} &K' \ar[r]^{l'} \ar[d]_{u'}& L'\ar[d]^{i_2} \\P \ar[r]_{p'}& D' & P \ar[r]_{p} & D}\]
	In particular, 	$\der{D}\updownarrow \der{D'}$. On the other hand, if $(v,v')$ with $v:K\to P$ and $v':K'\to P$ is another filler, then there are other  two pushout squares
	\[\xymatrix{K \ar[r]^{r} \ar[d]_{v}& R \ar[d]^{j_1} &K' \ar[r]^{l'} \ar[d]_{v'}& L'\ar[d]^{j_2} \\P \ar[r]_{p'}& D' & P \ar[r]_{p} & D}\]
	such that $f'\circ j_1=h$, $g\circ j_2=n'$. But then $(j_1, j_2)$ is an independence pair and so $j_1=i_1$ and $j_2=i_2$. \Cref{rem:unic} now yields the thesis.
	
	\smallskip \noindent $(2\Rightarrow 1)$ Let $(u,u')$ be the filler between $\dder{D}$ and $\dder{D}$'. Then we get an independence pair $(i_1, i_2)$ pushing out $u$ along $r$ and $u'$ along $l'$, respectively. If $(j_1, j_2)$ is another independence pair
\end{proof}
...
\begin{corollary} Given a tame  left-linear DPO-rewriting system  $(\X,\R)$ with $\X$ an $\mathcal{M}$-adhesive category and a derivation $\der{D}=\{\dder{D}_i\}_{i=0}^2$, the following are true: 
	\begin{enumerate}
		\item suppose that $(u,u')$ and $(w,w')$ are fillers witnessing  $\dder{D}_1\updownarrow_! \dder{D}_2$ and $\dder{D}_0\updownarrow_! S_{u,u'}(\dder{D}_2)$ respectively, if $\dder{D}_0\updownarrow_!\dder{D}_1$, then $S_{w,w'}(\dder{D}_0)\updownarrow_!S_{u,u'}(\dder{D}_1)$;
		\item 
	\end{enumerate}
\end{corollary}
\begin{proof}
	\begin{enumerate}
		\item Let $(v,v')$ be the unique filler between $\dder{D}_0$ and $\dder{D}_1$.  By \Cref{rem:filip}
		\item \qed 
	\end{enumerate}
\end{proof}
