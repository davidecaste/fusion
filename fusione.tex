...
\begin{proposition}Let $\X$ be an $\mathcal{M}$-adhesive category and $(\X, R)$ a linear DPO-rewriting system. Given two direct derivations $\dder{D}:G\Mapsto H$ and $\dder{D}':H\Mapsto T$, the following are equivalent:
	\begin{enumerate}
...
\item $\dder{D}'_j\cdot \dder{D}'_{j+1} = S_{u,u'}(\dder{D}, \dder{D}')$.
	...
	We will also say that $\der{D}$ is \emph{switch equivalent} to $\der{D}'$, if there exists a sequence, $\{\der{D}_i\}_{i=0}^n$ of derivations such that
	\begin{enumerate}
...
\end{proof}
\begin{corollary}
	\todo{ho davver una relazione di equivalenza}
\end{corollary}
