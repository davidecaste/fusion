...
\begin{proof}
	$(\Rightarrow)$ By hypothesis there exists $k:D'\to B$ such that $n\circ k = d$. By \Cref{prop:pbpo}, the bottom face of the cube is a pullback. Thus there exists a unique $h:C'\to A$ as in the diagram below, implying the thesis.
		\[\xymatrix@C=40pt{C'  \ar@{.>}[d]^{h}\ar[r]^{g'} \ar@/_.5cm/[dd]_{c}& D' \ar@/^.5cm/[dd]^{d}\ar[d]_{k}\\ A \ar[d]^{m} \ar[r]^{f} & B \ar[d]_{n} \\  C \ar[r]_g & D}\]
	...
	$(\Leftarrow)$ Let $h:C\to A$ be such that $c=m\circ h$. By the Van Kampen property the top face of the given cube is a pushout. Thus the dotted $k:D'\to B$ in the following diagram exists.
		\[\xymatrix@C=40pt{A' \ar@/_.5cm/[dd]_{a} \ar[d]^{m'} \ar[r]^{f'} & B' \ar[d]_{n'} \ar@/^.5cm/[dd]^{b} \\  C' \ar[d]^{h} \ar[r]_{g'} & D' \ar@{.>}[d]_{k}\\ A \ar[r]_f& B }\]
...
Given  two objects $G$ and $H$ and a rule $\rho=(l,r)$ in $\R$, a \emph{direct derivation $\mathscr{D}$ from $G$ to $H$ applying the rule $\rho$}, is a diagram as the one below, in which both squares are pushouts. 
	...
	We will denote a direct derivation $\dder{D}$ between $G$ and $H$ as $\dder{D}:G\Mapsto H$. W
\end{definition}
...
\begin{remark}\label{exa:conc} Let  $\dder{D}:G\Mapsto H$ be the direct derivation 
		\[\xymatrix{L \ar[d]_{n}& K \ar[d]^{k}\ar[l]_{l} \ar[r]^{r} & R \ar[d]^{h}\\G & \ar[l]^{f} D \ar[r]_{g}& H}\]
...
\[\xymatrix@C=40pt{&&D' \ar[r]^{g'} \ar@/_.45cm/[dll]_{f'}&H'\\G & L \ar[l]_{n} & K \ar[u]_{k'} \ar[d]^{k}\ar[r]^{r} \ar[l]_{l} &R\ar[u]^{h'} \ar[d]_{h}\\&&D\ar@/^.45cm/[ull]^{f}\ar@/^.4cm/@{.>}[uu]^(.4){t}|\hole \ar[r]_{g}&H\ar@/_.4cm/@{.>}[uu]_{s}}\]
\end{proposition}
\begin{proof}
	...
	\end{align*}
	Hence, we have the wanted $s:H\to H'$. To see that $s$ is an isomorphism, consider the diagram 
...
\begin{definition} Let $(\X, \R)$ be a a left-linear DPO-rewriting system. Given a derivation $\der{D}=\{\dder{D}_{i}\}_{i=0}^n$ between $G$ and $H$ and isomorphisms $\phi: G\to G'$, $\psi:H\to H'$, the derivations  $\phi*\der{D}$ and $\der{D}*\psi$ are defined as
...
\section{On rewriting systems on $\Set$.}\label{app:set}
