...
\begin{remark} \cite{baldan2011adhesivity} identifies a large class of (quasi)adhesive categories with the property that every left linear DPO-rewriting system on them is tame. We reproduce these results in \Cref{app:fill}. 
\end{remark}
...
\begin{proof}
	\begin{enumerate}
		\item Let $(i_1,i_2)$ be the unique independence pair between $\dder{D}_0$ and $\dder{D}_1$.  By tameness and \Cref{lem:iig1} we know that $S_{e_0,e_1}(\dder{D}_0)\updownarrow S_{a_1,a_2}(\dder{D}_1)$. Let $(\alpha_1, \alpha_2)$ be the independence pair constructed in the proof of \Cref{lem:iig1}. Take another independence pair $(\beta_1, \beta_2)$,  by \Cref{rem:weak} we already know that $\beta_1=\alpha_1$. Now, using the same notation as the proof of \Cref{lem:iig1}, we have 
		\[x_2\circ \beta_1 = y_1\circ e_0 \qquad z'_1\circ \beta_2 = y_1\circ b_1\]
		
		In particular, this implies that there exist $\beta'_1:R_0\to P_2$, $\beta'_2:L_1\to P_3$ as in the diagrams below.
		\[\xymatrix{R_0 &&& L_1\\ & P_2 & Q_3 && P } \]
