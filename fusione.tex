...
\author{Paolo Baldan \and Davide Castelnovo  \and  Andrea Corradini  Fabio Gadducci}
...
\end{remark}
	...
	Moreover,  if $\mathcal{D}_i$ is an application of the rule $\rho_i\in R$, then we can define an associated sequence of rules as $r(\der{D})$ as $\{\rho_i\}_{i=0}^n$.
...
\begin{definition}
	The \emph{DPO-derivation category} $\dpo$ of a left-linear DPO-rewriting system $(\X, \R)$ is the free category on $\gpo$.
\end{definition}
\begin{remark}
...
It is immediate to see that the concatenation of two non-empty derivations coincides with their composition in $\dpo$. 
\end{remark}
...
\end{proposition}
\begin{proof}
	For every object $X\in \X$, pick a single representative $\pi(X)$ of its isomorphism class. Let $\ske$ be the full subcategory given by these objects. By definition $\ske$ is skeletal and the inclusion functor is full, faithful and essentially surjective.\qed 
\end{proof}
\begin{remark}
...
The previous two identities show that $\psi^{n+1}$ is an abstract equivalence between $\dder{E}_0$ and $\dder{E}'_0$. By hypothesis $\pi(H)=\pi(E)$ and, moreover, we have
	...
	\end{split}\]
Therefore the thesis now follows.	\qed 
\end{proof}
...
\begin{proof} We split the cases.
	\begin{itemize}
		...
		Point $2$ of \Cref{lem:colim} entails that $\iota'_{G_j}$ is in $\mathcal{M}$, and so is $f_j$ as it is the pushout of $l_j$. Thus the rectangle we are considering is an $\mathcal{M}$-pushout and the thesis follows again from \Cref{prop:pbpoad}. \qed
	\end{itemize}
\end{proof}	
...
Notice that if $\dder{D}$ and $\dder{D}'$ are switchable, then they are weakly sequentially independent: indeed, if a filler between them exists, then $(i_1, i_2)$ is an independence pair.

\begin{definition}We will say that a left-linear DPO-rewriting system  $(\X, \R)$ is \emph{tame} if for every two direct derivations $\dder{D}:H\Mapsto H$, $\dder{D}':H\Mapsto T$ and for every independence pair $(i_1, i_2)$ between them, there exists a filler $(u,u')$ between them such that  the squares below are pushouts.\[\xymatrix{K \ar[r]^{r} \ar[d]_{u}& R \ar[d]^{i_1} &K' \ar[r]^{l'} \ar[d]_{u'}& L'\ar[d]^{i_2} \\P \ar[r]_{p'}& D' & P \ar[r]_{p} & D}\]
\end{definition}


\begin{proposition}\label{prop:equi}Every linear DPO-rewriting system. $(\X, \R)$ a linear is tame.
 \end{proposition}
\begin{proof} Let $(i_1, i_2)$ be an independence pair between $\dder{D}:G\Mapsto H$ and $\dder{D}':H\Mapsto T$. We have a diagram
	\[\xymatrix@C=15pt{L \ar[d]_{n}&& K \ar[d]_{k}\ar[ll]_{l} \ar[r]^{r} & R \ar@/^.35cm/[drrr]|(.3)\hole_(.4){i_1} \ar[dr]|(.3)\hole_{h} && L' \ar@/_.35cm/[dlll]^(.4){i_2} \ar[dl]|(.3)\hole^{n'}& K' \ar[d]^{k'}\ar[l]_{l'} \ar[rr]^{r'} && R' \ar[d]^{h'}\\G && \ar[ll]^{f} D \ar[rr]_{g}&& H  && \ar[ll]^{f'} D' \ar[rr]_{g'}&& T}\]
...
\begin{lemma}Let $(\X,\R)$ be a tame left-linear DPO-rewriting system with $\X$ an $\mathcal{M}$-adhesive category. Consider a derivation $\der{D}=\{\dder{D}_i\}_{i=0}^2$. If $(v,v')$ is a filler between $\dder{D}_0$ and $\dder{D}_1$, $(u,u')$ one between $\dder{D}_1$ and $\dder{D}_2$ and $\dder{D}_0\updownarrow S_{u,u'}(\dder{D}_2)$ with filler $(w,w')$, then there exists a filler between $S_{w,w'}(\dder{D}_0)$ and $S_{u,u'}(\dder{D}_1)$.
\end{lemma}
\begin{proof}
We can use \Cref{def:filler,def:switch} to get some diagrams.  First of all the filler $(v,v')$ between $\dder{D}_0$ and $\dder{D}_1$ yields
	...
	Secondly, the filler $(u,u')$ between $\dder{D}_1$ and $\dder{D}_2$ gives us another diagram:
\[
	...
	Finally, the filler $(w,w')$ between $\dder{D}_0$ and $S_{u,u'}(\dder{D}_2)$ provides us with:
		...
		By tameness of $(\X,\R)$, to construct a filler between $S_{w,w'}(\dder{D}_0)$ and $S_{u,u'}(\dder{D}_1)$, it is enough to construct the two dotted arrows in the diagram below.
		...
		\end{align*}
			implying the existence of $\alpha'_2:L_1\to P_3$ fitting in the fiagram below.
			\[\xymatrix{L_1 \ar@{.>}[dr]^{\alpha'_2} \ar@/^.3cm/[drr]^{i_1} \ar@/_.3cm/[ddr]_{e_0}\\ &P_2 \ar[r]^{c_2} \ar[d]_{d_1}& D_1 \ar[d]^{f_1}\\ &Q_2\ar[r]_{x_1} & G_1}\]
		
		\iffalse 
		  we can start pulling back $x_2:Q_3\to G'_2$, which is in $\mathcal{M}$, along $z'_1:Q_5\to G'_2$ along  to get the solid part of the diagram below.
			\[\xymatrix{&&R_0 \ar@{.>}@/_1cm/[ddrr]_(.25){\alpha_1}|(.7)\hole  \ar[dr]^{\hspace{-4pt}y_1\circ e_0}&& L_1\ar@{.>}@/^1cm/[ddll]^(.25){\alpha_2}  \ar[dl]_{y_1\circ b_1\hspace{-4pt}}\\&K_0\ar[dr]^{n_1\circ w}\ar[dl]_{l_0} \ar@{.>}@/_.8cm/[ddrr]_(.65){\beta}\ar[ur]^{r_0}&& G'_2&& K_1 \ar@{.>}@/^.8cm/[ddll]^(.65){\beta'}\ar[dl]_{d_2\circ u}\ar[lu]_{l_1} \ar[dr]^{r_1}\\L_0 \ar[dr]_{z'_0\circ o_0} && Q_5 \ar[dl]|(.4)\hole_(.6){z_1}\ar[ur]|(.5)\hole^(.7){z'_1}&&Q_3 \ar[dr]|(.4)\hole^(.65){y_2} \ar[ul]|(.5)\hole_(.7){x_2}&&R_1\ar@{.>}@/^.8cm/[ddll]^{\gamma'}\ar[dl]_(.4){g_2\circ a_1}\\&G'_1 &&P_4\ar@{.>}[dr]^{\epsilon'} \ar[ur]^(.4){\delta'}\ar[ul]_(.4){\delta}&&G_2\\&& &&Q_6}\]
		
		Let us construct the dotted arrows.
		\begin{itemize}
		\item Existence of $\beta$. Consider the arrows $i_0\circ r_0:K_0\to D_1$ and $u_1\circ w:K_0\to Q_2$. Then
		\begin{align*}
	f_1\circ i_0\circ r_0&=h_0\circ r_0\\&=g_0\circ k_0\\&=g_0\circ u_0\circ w\\&=x_1\circ u_1\circ w
		\end{align*}
		Therefore there exists a unique $\underline{\beta}:K_0\to P_2$ making the diagram 
		\[\xymatrix{K_0 \ar@{.>}[dr]^{\underline{\beta}} \ar[r]^{r_0} \ar[d]_{w}& R_0 \ar@/^.2cm/[dr]^{i_0}\\ P_3 \ar@/_.2cm/[dr]_{u_1}&P_2 \ar[r]^{c_1} \ar[d]_{d_1}& D_1 \ar[d]^{f_1}\\ &Q_2\ar[r]_{x_1} & G_1}\]
		commutative. Computing further we also get
		\begin{align*}
		x_2\circ d_2\circ \underline{\beta}&=y_1\circ d_1\circ \underline{\beta}\\&=y_1\circ u_1\circ w\\&=z'_1\circ u_1\circ w
		\end{align*}
		We conclude that there exists the dotted  $\beta: K_0\to P_4$ depicted in the diagram below.
		\[\xymatrix{K_0 \ar@{.>}[dr]^{\beta} \ar[r]^{\underline{\beta}} \ar[d]_{w}& P_2 \ar@/^.2cm/[dr]^{d_2}\\ P_3 \ar@/_.2cm/[dr]_{n_1}&P_4 \ar[r]^{\delta'} \ar[d]_{\delta}& Q_3 \ar[d]^{x_2}\\ &Q_5\ar[r]_{z'_1} & G'_2}\]
		\item Existence of $\beta'$. We proceed as above, this time taking the arrows $i_1\circ l_1:K_1\to D_0$ and $d_1\circ u:K_1\to Q_2$. We get
		\begin{align*}
			g_0\circ i_1\circ l_1&=m_1\circ l_1\\&=f_1\circ k_1\\&=f_1\circ c_1\circ u\\&=x_1\circ d_1\circ u
		\end{align*}
		giving a unique $\underline{\beta}':K_1\to P_3$ fitting in the diagram 
		\[\xymatrix{K_1 \ar@{.>}[dr]^{\underline{\beta}'} \ar[r]^{l_1} \ar[d]_{u}& L_1 \ar@/^.2cm/[dr]^{i_1}\\ P_2 \ar@/_.2cm/[dr]_{d_1}&P_3 \ar[r]^{u_0} \ar[d]_{u_1}& D_0 \ar[d]^{g_0}\\ &Q_2\ar[r]_{x_1} & G_1}\]
		If we compute we get
		\begin{align*}
			z'_1\circ n_1\circ\underline{\beta}'&=y_1\circ u_1\circ \underline{\beta}'\\&=y_1\circ d_1\circ u\\&=x_2\circ d_2\circ u
		\end{align*}
		Therefore there is $\beta':K_1\to P_4$ such that
		\[\xymatrix{K_1 \ar@{.>}[dr]^{\beta'} \ar[r]^{\underline{\beta}'} \ar[d]_{u}& P_3 \ar@/^.2cm/[dr]^{n_1}\\ P_2 \ar@/_.2cm/[dr]_{d_2}&P_4 \ar[r]^{\delta} \ar[d]_{\delta'}& Q_5 \ar[d]^{z'_1}\\ &Q_3\ar[r]_{x_2} & G'_2}\]
		commutes.
		\item Existence of $\alpha_1$. Let us consider the arrows $i_0:R_0\to D_1$ and $e_0:R_0\to Q_2$. An easy computation shows that
		\begin{align*}
			f_1\circ i_0&= h_0\\&=x_1\circ e_0
		\end{align*}
	entailing the existence of  $\alpha'_1:R_0\to P_2$
	\[\xymatrix{R_0 \ar@{.>}[dr]^{\alpha'_1} \ar@/^.3cm/[drr]^{i_0} \ar@/_.3cm/[ddr]_{e_0}\\ &P_2 \ar[r]^{c_2} \ar[d]_{d_1}& D_1 \ar[d]^{f_1}\\ &Q_2\ar[r]_{x_1} & G_1}\]
	We can then define $\alpha_1:R_0\to Q_3$ as $d_2\circ \alpha'_1$. Notice that 
	\begin{align*}
x_2\circ \alpha_1&=x_2\circ d_2\circ \alpha'_1\\&= y_1\circ d_1\circ \alpha'_1\\&=y_1\circ e_0
	\end{align*}
This equality, in turn, implies	
		\begin{align*}
x_2\circ \alpha_1\circ r_0&=y_1\circ e_0\circ r_0\\&=y_1\circ  u_1\circ w \\&= z'_1\circ  n_1\circ w \\&= z'_1 \circ \beta \circ \delta \\&= x_2\circ \beta \circ \delta' 
	\end{align*}
	Since $x_2$ is mono, because $x_2\in \mathcal{M}$, we conclude that the square below is commutative.
	\[\xymatrix{K_0 \ar[r]^{r_0} \ar[d]_{\beta}& R_0  \ar[d]^{\alpha_1}\\ P_4 \ar[r]_{\delta'} & Q_3}\]
	
		\item Existence of $\alpha_2$.
		\item Existence of $\epsilon'$.
		\item Existence of $\gamma'$.
		\end{itemize}
		
		Finally, we have to show that these data form a filler.
	\begin{enumerate}
		\item \todo{This follows by definition}
		\item We have to show that the following two squares are pushouts
		\[\xymatrix{}\]
		
		\smallskip
		For the first one, consider the diagram
		\[\xymatrix{K_0 \ar[r]^{r_0} \ar[d]_{\beta}& R_0  \ar[d]^{\alpha_1}\\ P_4 \ar[r]_{\delta'} & Q_3\\&& U}\]
			\[\xymatrix{K_0 \ar[r]^{r_0} \ar[d]_{\beta}& R_0  \ar[d]^{\alpha_1}\\ P_4 \ar[r]_{\delta'} & Q_3}\]
...
\begin{lemma}\todo{nell'altro senso}
\end{lemma}
\begin{proof}
 \todo{fare} \qed 
\end{proof}
...
\begin{corollary}[Invariance under shift]
	contenuto...
\end{corollary}
