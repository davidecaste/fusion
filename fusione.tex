...
\begin{definition}[Consistent permutation]\label{def:permcon}
	...
	\[\xymatrix@C=30pt{\pi(G_0)\ar[r]^{\alpha} \ar[d]_{\alpha'} & G_0 \ar[r]^{\iota_{G_0}} &\tpro{D} \ar[d]^{\xi_\sigma}&   \pi(G_{n+1}) \ar[r]^{\omega} \ar[d]_{\omega'} & G_{n+1} \ar[r]^{\iota_{G_{n+1}}} &\tpro{D} \ar[d]^{\xi_\sigma}\\ G'_0 \ar[rr]_{\iota'_{G'_0}} & &\lpro \der{D}' \rpro& G'_{n+1} \ar[rr]_{\iota'_{G'_{n+1}}} & &\lpro \der{D}' \rpro}\]
...
\begin{example}\todo{identità consistente non implica astrazione}
\end{example}
...
If $\sigma_1\colon [0, \lgh(\der{D}_1)-1]\to [0, \lgh(\der{D}_1)-1]$ is a consistent permutation between  $(\der{D}_1, \alpha_1, \omega_1)$ and $(\der{D}'_1, \alpha'_1, \omega'_1)$ and $\sigma_2:[0, \lgh(\der{D}_2)-1]\to [0, \lgh(\der{D}_2)-1]$ one between $(\der{D}_2, \alpha_2, \omega_2)$ and $(\der{D}'_2, \alpha'_2, \omega'_2)$, then 
\[\sigma_1+\sigma_2\colon[0, \lgh(\der{D})-1]\to[0, \lgh(\der{D})-1] \qquad n \mapsto \begin{cases}
	\sigma_1(n) & n < \lgh(\der{D}_1)\\
	\sigma_2(n-\lgh(\der{D}_1)) &   \lgh(\der{D}_1) \leq n
\end{cases}\]
...
\begin{proof}
	...
	Following \Cref{def:conc}, we will split the proof in three cases.
	\begin{itemize}
		\item $\lgh(\der{D}_1)=0$. Thus $\lgh(\der{D}'_1)=0$ too  and we have
		\[(\der{D}, \alpha, \omega)=(\der{D}_2, \alpha_2\circ \omega_1^{-1}\circ \alpha_1, \omega_2) \quad (\der{D}', \alpha', \omega')=(\der{D}'_2, \alpha'_2\circ (\omega'_1)^{-1}\circ \alpha'_1, \omega'_2)\]
	Moreover, in this case $\sigma_1+\sigma_2=\sigma_2$. The thesis now follows at once noticing that 
	\begin{align*}\xi_{\sigma_2}\circ \iota_{G_{2,0}}\circ \alpha_2 \circ \omega_1^{-1}\circ \alpha_1 &=\iota'_{G'_{2,0}} \circ \alpha'_2 \circ \omega^{-1}_1 \circ \alpha_1\\&=\iota'_{G'_{2,0}} \circ \alpha'_2 \circ (\omega'_1)^{-1}\circ \xi_{\id{\emptyset}} \circ \alpha_1\\&=\iota'_{G'_{2,0}} \circ \alpha'_2\circ (\omega'_1)^{-1} \circ \alpha'_1
	\end{align*}
