...
By hypothesis we have $g\colon f^*(n)\to m$ in $\mathcal{M}/X$, thus \Cref{lem:varie} yields a unique morphism $\bar{g}\colon n\to k$ in $\mathcal{M}/Y$. By the commutativity of the diagram
\[\xymatrix{f^*(N) \ar[r]^{f^*(\bar{g})}& f^*(Q) \ar[dr]^{\epsilon_k} \ar@{>->}@/^.2cm/[drr]^{f^*(k)} \ar@/_.2cm/[ddr]_{p}\\&& M \ar@{>->}[r]^{m}\ar[d]_{h}& X \ar[d]^f \\&N &Q\ar@{>->}[r]_{k}&Y}\]
we can deduce that $\epsilon_k \circ $ as wanted.
\end{proof}

We can deduce a host of properties of pushout complements from \Cref{lem:part}.
...
\subsubsection{Factorization systems and $\mathcal{M}$-adhesivity} \todo{Da sistemare}
