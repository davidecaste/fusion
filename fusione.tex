...
\begin{lemma}\label{lem:colim}
	...
	\begin{enumerate}
		\item if $\der{D}$ is a derivation of length one given by the following diagram
		
		then the colimiting cocone $(\tpro{D}, \{\iota_X\}_{X\in \Delta(\der{D})})$ exists and  fits in the diagram below, wher the bottom face is a pushout;
		\[\xymatrix{}\]
		\item 
		\item  for every non-empty derivation $\der{D}:G\to H$, the diagram $\Delta(\der{D})$ has a colimit $(\tpro{D}, \{\iota_X\}_{X\in \Delta(\der{D})})$ such that $\iota_H$ belong to $\mathcal{M}$.
	\end{enumerate}
\end{lemma}
...
\begin{lemma}	Let $\X$ be an $\mathcal{M}$-adhesive category and $(\X, R)$ a left-linear DPO-rewriting system over it.
 Then
\end{lemma}
\begin{proof}
