...
We will say that  two decorated derivations$(\der{D}, \alpha, \omega)$ and $(\der{D}', \alpha', \omega')$ are \emph{abstraction equivalent}, if $\lgh(\der{D})=\lgh(\der{D}')$, $r(\der{D})=r(\der{D'})$, and there exists a family $\{\phi^i\}_{i=0}^{\lgh(\der{D})}$ of abstraction equivalences $\phi^i$ between $\dder{D}_i$ and  $\dder{D}'_i$ such that, for $i\in [0, \lgh(\der{D})]$, $\phi^i_3=\phi^{i+1}_1$ and  the following diagrams commute.
...
We will use $\equiv^a$ to denote the resulting equivalence relation. An \emph{abstract decorated derivation}.  $[\der{D}, \alpha, \omega]_a$ is just the equivalence class of  $(\der{D}, \alpha, \omega)$.
\end{definition}
