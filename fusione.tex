...
\newcommand{\sder}[2]{I(\mathscr{#1}, \mathscr{#2})}
...
\begin{theorem}[\cite{CastelnovoGM22,ehrig2006fundamentals}]\label{lem:comma}
	Let $\A$ and $\B$ be respectively an $\mathcal{M}$-adhesive and an $\mathcal{M}'$-adhesive category. Let also $L:\A\rightarrow \C$ be a functor that preserves $\mathcal{M}, \mathcal{N}$-pushouts, and  $R:\B\rightarrow \C$ be a functor which preserves pullbacks.Then $\comma{L}{R}$ is $\cma{M}{M'}$-adhesive, where 
	\[
	\cma{M}{M}':=\{(h,k)\in \mathcal{A}(\comma{L}{R}) \mid h\in \mathcal{M}, k\in \mathcal{M}'\}\]
\end{theorem}

In particular, we can apply this result  to slices over a given object.

\begin{corollary}\label{cor:slice}
	Let  $X$ be an object of an $\mathcal{M}$-adhesive category $\X$. Then  $\X/X$ is $\mathcal{M}/X$-adhesive, where
...
\begin{example}Topos, ipergrafi e grafi
\end{example}


\begin{example}GRAFI GERARCHICI
\end{example}

\begin{example}term graph
\end{example}
...
\begin{proof}
\begin{enumerate}
	\item 	We can begin noticing that $c$ is an element of $\mathcal{M}$ because it is the pushout of $a$. Moreover, $b$, being the pullback of $c$, is in $\mathcal{M}$ too. Now, the whole rectangle is a $\mathcal{M}$-pushout, thus, by \Cref{prop:pbpoad}, it is a pullback and so its left half is a pullback, by \Cref{prop:pbpo}. Moreover, $g$, being the pullback of a monomorphism, is monic too.
	
	Putting together these observations, we can build the cube below, in which all the vertical faces are pullbacks.
	...
	The bottom face is a Van Kampen square and so its top one is a pushout. The thesis now follows from \Cref{lem:po1}.
	...
	Using again \Cref{lem:po1}, it is enough to show that the left half of the original rectangle is a pushout. We can build the same cube of the previous point:
	\[\xymatrix@C=20pt@R=10pt{ & &X\ar[dl]_{f} \ar[ddd]^(.333333){\id{X}}|(.666666)\hole\ar[rrr]^{a} &&&A \ar[ddd]^{\id{A}} \ar[dl]^{h}\\& Y\ar[dl]_{\id{Y}} \ar[ddd]|(.333333)\hole_{\id{Y}} &&&B \ar[ddd]^{\id{B}} \ar[dl]_{\id{B}} \\Y \ar[ddd]_{g} \ar[rrr]^{b}&&& B\ar[ddd]^{k}\\&&X \ar[rrr]|(.34)\hole^{a}|(.67)\hole \ar[dl]_{f}&&& A \ar[dl]^{h}\\ & Y \ar[rrr]|(.67)\hole^{b} \ar[dl]_{g}&&& B \ar[dl]^{k}\\ Z\ar[rrr]_{c} &&& C}\]
...
A \emph{left-linear DPO-rewriting system} is a pair $(\X, R)$ where $\X$ is a $\mathcal{M}$-adhesive category and $R$ is a set of left $\mathcal{M}$-linear rules. Given  two objects $G$ and $H$ and a rule $\rho=(l,r)$ in $R$, a \emph{direct derivation $\mathscr{D}$ from $G$ to $H$ via $\rho$}, is a diagram as the one below, in which both squares are pushouts. 
...
Then there exist isomorphisms $t:D\to D'$ and $s:H\to H'$ as in the following diagram.
	\[\xymatrix@C=30pt@R=10pt{ &&&R\ar[dddr]^{h'} \ar[dddl]_{h}|(.67)\hole\\&&K \ar[dl]_{l} \ar[ur]^{r}\ar[dddr]^{k'} \ar[dddl]_{k}\\&L \ar[dddl]_{n}\\&&H \ar@{.>}[rr]^{s} && H'\\ & D \ar[ur]^{g} \ar@{.>}[rr]^{t} \ar[dl]_{f}&& D' \ar[ur]_{g'}\ar@/^.3cm/[dlll]^{f'}\\ G}\]
\end{proposition}
\begin{proof}
...
Given two derivations $\der{D}=\{\dder{D}_i\}_{i=0}^n$ and $\der{D}'=\{\dder{D}'_i\}_{i=0}^n$ such that $\dder{D}_i:G_i\Mapsto G_{i+1}$ and $\dder{D}'_i:G'_i\Mapsto G'_{i+1}$., we will say that $\der{D}$ and $\der{D}'$ are \emph{abstraction equivalent}, if  $r(\der{D})=r(\der{D'})$  and there exists a family $\{\phi^i\}_{i=0}^n$ of abstraction equivalences $\phi^i:\dder{D}_i\to \dder{D}'_i$ such, for every index $i$, $\phi^i_3=\phi^{i+1}_1$.
	...
	In such situation we will say that $\der{D}$ and $\der{D}'$ are \emph{abstraction equivalent}. We will use $\equiv^a$ to denote the resulting equivalence relation, while $[\der{D}]_a$ will denote the equivalence class of $\der{D}$. 
\end{definition}
\begin{remark}
...
\begin{lemma}\cite{heindel2009category}
	Let $\X$ be an $\mathcal{M}, \mathcal{N}$-adhesive category and $(\X, R)$ a left-linear DPO-rewriting system over it. Then for every non-empty derivation $\der{D}:G\to H$, the diagram $\Delta(\der{D})$ has a colimit $(\tpro{D}, \{\iota_X\}_{X\in \Delta(\der{D})})$ such that $\iota_H$ belong to $\mathcal{M}$.
\end{lemma}
\begin{proof}
	We proceed by induction.
	\begin{enumerate}
		\item Suppose that $n=0$ so that $\der{D}$ consists only of the following direct derivation
		\[\xymatrix{L \ar[d]_{n}& K \ar[d]^{k}\ar[l]_{l} \ar[r]^{r} & R \ar[d]^{h} \\G & \ar[l]^{f} D \ar[r]_{g}& H }\]
		then $f\in \mathcal{M}$ so that we can consider its pushout along $g$ to get a diagram
		\[\xymatrix{L \ar[d]_{n}& K \ar[d]^{k}\ar[l]_{l} \ar[r]^{r} & R \ar[d]^{h} \\G \ar@/_.2cm/[dr]_{q}& \ar[l]^{f} D \ar[r]_{g}& H  \ar@/^.2cm/[dl]^{p}\\ & P}\]
		Let $(C, \{c_X\}_{X\in \Delta(\der{D})})$ be another cocone, in particular we have
		\begin{align*}
			c_H\circ g & = c_D \\&= c_G\circ f
		\end{align*}
		Hence, there exists a unique $c:P\to C$ such that 
		\[c\circ q= c_G \qquad c\circ p =c_H\]
		Now, $f$ is in $\mathcal{M}$ as it is the pushout of $l$, hence $p$ is in $\mathcal{M}$ too and this gives us the thesis. 
		...
		\[\xymatrix{L_n \ar[d]_{m_{n}}& K_{n} \ar[d]^{k_{n}}\ar[l]_{l_{n}} \ar[r]^{r_{n}} & R_{n} \ar[d]^{h_n} \\G_{n} \ar[d]_{\iota_{G_{n}}}& \ar[l]^{f_n} D_n \ar[r]_{g_n}& H  \ar[d]^{q}\\ \lpro \der{D}' \ar[rr]_{p}\rpro && P}\] 
...
\begin{example}\todo{esempi di cose equivalenti}
\end{example}

\begin{lemma}\label{lem:unique}\todo{Questo è il punto 2 di Lemma 17}
	contenuto...
\end{lemma}
\begin{proof}
	contenuto...
...
\subsection{Sequentially independend ndependent and switchable derivations}
\todo{A VERY NICE INTRO}
...
\[\xymatrix@C=12pt{L \ar[d]_{n}&& K \ar[d]_{k}\ar[ll]_{l} \ar[r]^{r} & R \ar@/^.35cm/@{.>}[drrr]_(.4){i_1} \ar[dr]|(.3)\hole_{h} && L' \ar@/_.35cm/@{.>}[dlll]^(.4){i_2} \ar[dl]|(.3)\hole^{n'}& K' \ar[d]^{k'}\ar[l]_{l'} \ar[rr]^{r'} && R' \ar[d]^{h'}\\G && \ar[ll]^{f} D \ar[rr]_{g}&& H  && \ar[ll]^{f'} D' \ar[rr]_{g'}&& T}\]
...
\begin{remark} Let $(i_1, i_2)$ and $(j_1, j_2)$ be independence pairs for the direct derivations $\dder{D}$ and $\dder{D'}$. Notice that, by definition, we have
	\begin{align*}f'\circ i_1&=h\\&=f'\circ j_1
	\end{align*}
	On the other hand, 
	 $f':D'\to H$ is  the pushout of $l':K'\to L'$ and so it is in $\mathcal{M}$, implying $j_1=i_1$.
	 
	 On the other hand, if the rules $\rho$ applied in $\dder{D}$ is linear, then $g:D\to H$ is in $\mathcal{M}$, hence, from the equation
	 \begin{align*}
	 g\circ i_2&=h \\&= g\circ j_2
	 \end{align*}
we can deduce that $i_2=j_2$, too.

Summing up, it all the rules of $(\X, R)$ are linear, then sequential independence and weak sequential independence coincide. 
\end{remark}
...
A \emph{filler} $\dder{D}$ and $\dder{D}'$ is  given by a pair of arrows $u:K\to P$ and $u':K'\to P$ satisfying the following conditions
\begin{enumerate}
...
We will say that $\dder{D}$ and $\dder{D}'$ are \emph{switchable} if a filler between them exists. If such a filler is unique, we will say that $\dder{D}$ and $\dder{D}'$ are \emph{properly switchable}. We will use the notation $\dder{D}\updownarrow \dder{D'}$ to mean that $\dder{D}$ and $\dder{D}'$ are switchable, while $\dder{D}\Updownarrow \dder{D'}$ will denote that they are properly so.
\end{definition}
...
\begin{remark}\label{rem:deco} Let $\dder{D}$ and $\dder{D'}$ be two switchable direct derivations. Then the existence of a filler allows us to build the solid part of the diagram below.
	\[\xymatrix{&&R \ar@/_1cm/[ddrr]_(.35){i_1} \ar[dr]^{h}&& L'\ar@/^1cm/[ddll]^(.35){i_2}  \ar[dl]_{n'}\\&K\ar[dr]^{k}\ar[dl]_{l} \ar@/_.8cm/[ddrr]^(.65){u}\ar[ur]^{r}&& H && K' \ar@/^.8cm/[ddll]_(.65){u'}\ar[dl]_{k'}\ar[lu]_{l'} \ar[dr]^{r'}\\L \ar[dr]_{n} \ar@{.>}@/_.8cm/[ddrr]_{j'}&& D \ar[dl]|(.42)\hole_(.64){f}\ar[ur]|(.48)\hole^(.7){g}&&D' \ar[dr]|(.42)\hole^(.65){g'} \ar[ul]|(.48)\hole_(.7){f'}&&R'\ar@/^.8cm/[ddll]^{j}\ar[dl]^{h'}\\&G &&P\ar[dr]^{q'} \ar@{.>}[dl]_{q}\ar[ur]^(.4){p'}\ar[ul]_(.4){p}&&T\\&&Q \ar@{.>}[ul]_{s} &&Q'\ar@{.>}[ur]^{t}}\]
...
We are now going to justify the choice of the name for the relation $\updownarrow$, showing that two switchable direct derivations $\dder{D}$ and $\dder{D'}$ can be actually switched. Consider the following diagram: the solid part exists by the definition of a filler, while the two new dotted arrows $v:Q\to J$ and $v':Q'\to J$ are obtained  as the pushout of $q:P\to Q$, which is in $\mathcal{M}$ by \Cref{rem:deco}, along $q':P\to Q'$.
...
\begin{definition}
	Let $(\X,R)$ be a left-linear DPO-rewriting system and suppose that $\X$ is $\mathcal{M}$-adhesive. Given two switchable direct derivations $\dder{D}:G\Mapsto H$ and $\dder{D}':H\Mapsto T$, we can define other two direct derivations $I_{\dder{D}'}:G\Mapsto J$ and $I_{\dder{D}}:J\Mapsto T$ as follows:
	...
	The \emph{switching} $\sder{D}{D'}$ of $\dder{D}$ and $\dder{D'}$ is the derivation $I_{\dder{D'}} \cdot I_{\dder{D}}$.
\end{definition}

\begin{remark}Notice that $(j, j')$ is an independence pair for $I_{\dder{D}'}$ and $I_{\dder{D}}$. This is wintessed by the following diagram, commutative by construction.
...
\begin{example}\todo{controesempio sul fatto che scambiando posso avere debolezza}
\end{example}


\begin{example}\todo{esempio sul perché weakly independence/scambiabilità non basta nel caso non lineare}
\end{example}



\begin{proposition}bla blah 
		unicità del filler implica unicitò degli ind pair
\end{proposition}
\begin{proof}
	contenuto...\qed 
\end{proof}
...
\begin{definition}Let $\dder{D}:G\Mapsto H$ and $\dder{D}':H\Mapsto T$ be two direct derivations in a left-linear DPO-rewriting system $(\X, R)$. We will say that $\dder{D}$ and $\dder{D}'$ are \emph{properly interchangeable} if:
	\begin{enumerate}
		\item $\dder{D}$ and $\dder{D}'$ are sequentially independent;
		\item $I_{\dder{D}'}$ and $I_{\dder{D}}$ are sequentially independent too.
	\end{enumerate} 
	
	Take two derivations $\der{D}=\{\dder{D}_{i}\}_{i=0}^n$ and $\der{D}'=\{\dder{D}'_{i}\}_{i=0}^n$ with the same length and between the same $G_0$ and $G_n$. We say that $\der{D}'$ is \emph{obtained by a switch from $\der{D}$} if there exists an index $j\in n$ such that
	\begin{itemize}
\item 
\item 		
	\end{itemize}
...
\begin{lemma}\todo{Punto 1 del Lemma 17}
	contenuto...
\end{lemma}
\begin{proof}
	contenuto...
...
\begin{example}\todo{permutazione consistente non implica scambiabilità}
\end{example}
