...
\begin{example}\todo{identit� consistente non implica astrazione}Notice that the \Cref{ex:empty} cannot, in general, be generalized to non-empty derivations. A counterexample is the following one.
\end{example}
...
\section{Factorization systems}\label{app:fact}
Let us start by recalling the notion of \emph{factorization system} \cite{bousfield1977constructions,kelly1991note,rosicky2007factorization,tholen1983factorizations}.

\begin{definition}\label{def:fs}\index{factorization system} \index{left lifting property} \index{factorization system!proper -} \index{factorization system!stable -}
...
\begin{remark}\label{rem:dual}It is immediate to notice that, given a factorization system $(\mathcal{E}, \mathcal{M})$ on $\X$, then $(\mathcal{M}, \mathcal{E})$ is a factorization system on $\X^{op}$ which is proper if and only if $(\mathcal{E}, \mathcal{M})$ is so.  
\end{remark}

The following lemma assures us that the factorization of an arrow is unique up to isomorphism.

\begin{lemma}\label{lem:ima}
	Let $(\mathcal{E}, \mathcal{M})$ be a factorization system on a category $\X$. Then for every $X\in \X$ the inclusion functor $I_X\colon \mathcal{M}/X\to \X/X$ has a left adjoint.
\end{lemma}
\begin{proof}
	Given $f\colon Y\to X$ we know that there exist $e_f\colon Y\to E$ in $\mathcal{E}$ and $m_f\colon E\to X$ in $\mathcal{M}$ such that $f=m_f\circ e_f$. In particular, $e_f$ is a morphism $f\to m_f$ in $\X/X$.   Now, if $n\colon N\to X$ is another arrow in $\mathcal{M}$ and $g\colon f\to n$ a morphism in $\X/X$, then we have a rectangle
	\[\xymatrix@C=40pt{Y \ar[r]^{g}  \ar[dr]^(.35){f}|\hole \ar[d]_{e_f}& N \ar[d]^{n}\\ E \ar[r]_{m_f} \ar@{.>}[ur]_(.35){h}& X}\]
	By the left lifting property we get a unique dotted $h\colon m_f\to n$ fitting in the diagram above, giving us the thesis.
\end{proof}
...
\end{corollary}
\begin{proof}
	By the proof of \Cref{lem:ima} we know that there exist $L, L'\colon \X/X\to \mathcal{M}/X$ left adjoint to $I_X$ such that
	\[L(f)=m \qquad L'(f)=m'\]
	Moreover the units $\eta$ and $\eta'$ of the adjunctions $L\dashv R$ and $L'\dashv R$ are such that
	\[\eta_f= e \qquad \eta'_f=e'\]
	By the uniqueness of adjoints we get the wanted isomorphism.
\end{proof}

The previous corollary allows us to deduce other useful properties of factorization systems.

\begin{proposition}\label{cor:iso} Given a factorization system $(\mathcal{E}, \mathcal{M})$ on a category $\X$, the following hold:
	\begin{enumerate}
		\item an arrow $f\colon X\to Y$ is in $\mathcal{M}$ if and only if $e_f$ is an isomorphism;
...
\begin{proof}
	\begin{enumerate}
		...
		\smallskip \noindent
		($\Leftarrow$) This follows immediately since, if $e_f$ is an isomorphism then, $f$ is the composition of two arrows in $\mathcal{M}$.
		...
		\[(e_h\circ e_f)^{-1}\circ e_h \circ e_f =\id{X} \]
		so $e_f$ is a split monomorphism. The thesis now follows since $e_f$ is epic.
		\item 	Suppose that the square below is a pullback, with $m\in \mathcal{M}$
		\[\xymatrix{X\ar[r]^{g} \ar[d]_{n} & V \ar[d]^{m} \\ Y \ar[r]_{f}  & Z}\] 
		We can factor  $n$ as $m_n\circ e_n$ for $e_n\colon X\to E$ in  $\mathcal{E}$ and $m_n\colon E\to Y$ in $ \mathcal{M}$.  By the left lifting property we get the dotted $l\colon E\to X$ in the diagram below.
		\[\xymatrix{X \ar[d]_{e_n}\ar[rr]^{g} & &V\ar[d]^{m}\\  E  \ar@{.>}[urr]^{l} \ar[r]_{m_n}&Y\ar[r]_{f} & Z}\]
		Therefore we get another diagram
		\[\xymatrix{ E \ar@/^.5cm/[drr]^{l}\ar@{.>}[dr]^{k} \ar@/_.5cm/[ddr]_{m_n}\\ & X \ar[d]_{n}\ar[r]^{g}& V  \ar[d]^{m} \\ &Y \ar[r]_{f} & Z }\]
		and thus we can deduce the existence of the dotted $k\colon E\to V$. On the one hand, computing we get
		\[\begin{split}
			n\circ 	k\circ e_n&=m_n  \circ  e_n\\ &=n
		\end{split}\qquad \begin{split}
			g\circ k\circ e_n &=l\circ e_n\\ &=g \end{split}\]
		and so $k\circ e_n= \id{X}$.  On the other hand 
		\[\begin{split}
		...
		\xymatrix@C=50pt{V \ar[d]_{e_n}\ar[r]^{e_n}& E \ar[d]^{m_n} \\
			E\ar[ur]^{e_n\circ k}\ar[r]_{m_n} & Y}\]
		Hence, by the uniqueness clause of the left lifting property, $e_n\circ k=\id{E}$. Therefore $e_n$ is an isomorphism and the thesis follows from  point $1$. \qedhere 
	\end{enumerate}
\end{proof}
...
\subsubsection{$\mathcal{M}$-subobjects and factorization systems}

Let $\mathcal{M}$ be a class of monos in a category $\X$, then $\mathcal{M}/X(f,g)$ has cardinality at most one for every two arrows $f$ and $g$ with codomain $X$. Thus it is a preordered class, in which $f\leq g$ if and only if  $\mathcal{M}/X(f,g)$ is non-empty. Now, it is immediate to notice that $f\leq g$ and $g\leq f$, then there exists a (unique) isomorphism fitting in the triangle below.
...
\begin{remark}\label{rem:pbo} Let $f\colon X\to Y$ be an arrows admitting a pullback along arrows in $\mathcal{M}/Y$. The categories associated to $(\sub{M}{X}{X}, \leq)$ and $(\sub{M}{Y}{Y}, \leq)$ are equivalent to $\mathcal{M}/X$ and $\mathcal{M}/Y$, respectively, thus we get a monotone function $f^{-1}\colon \sub{M}{X}{Y}\to \sub{M}{X}{X}$ sending $[m]$ to $[f^*(m)]$.
\end{remark}

\begin{remark}\label{rem:size}
	Every $\mathcal{M}$-adhesive cateory $\X$ admits a full and faithful functor $F:\X\to \E$ into a topos $\E$, sending arrows in $\mathcal{M}$ to monomorphisms \cite{castelnovo2023thesis}. Since every topos is \emph{well-powered}, i.e. $\msub{E}{E}$ is a set for every object $E$ \cite{maclane2012sheaves}, we can deduce  that, for every $X$ in $\X$, $\sub{M}{X}{X}$ is actually a set too.
\end{remark}


Our next step is to compute finite infima in $(\sub{M}{X}{X}, \leq)$.

\begin{proposition}\label{prop:inf}
	Let $\mathcal{M}$ be a class of monos  in a category $\X$.  Let  $m\colon M\to X$ be an arrow in $\mathcal{M}$, then for every family $\{m_i\}_{i\in I}$ of sobjects in $\mathcal{M}/X$ the following are equivalent:
	\begin{enumerate}
		\item $[m]$ is the infimum of $\{[m_i]\}_{i\in I}$;
		\item there is a limiting cone $(m, \{k_i\}_{i\in I})$ for the diagram in $\mathcal{M}/X$ defined by the family $\{m_i\}_{i\in I}$.
	\end{enumerate}
\end{proposition}
\begin{proof}
	$(1) \Rightarrow (2)$ Let us denote by $M_i$ the domain of $m_i$. By hypothesis $m\leq m_i$, so that there exists $k_i\colon M\to M_i$. Let $(n, \{h_i\}_{i\in I})$ be another cone in $\mathcal{M}/X$, that is we have arrows $n\colon N\to X$ in $\mathcal{M}$ and $h_i\colon N\to M_i$. In particular $[n]$ is a lower bound in $(\sub{M}{X}{X}, \leq)$ for $\{[m_i]\}_{i\in I}$, thus there exists a, necessarily unique, arrow $t\colon N\to M$ such that $m\circ t=n$. Now, for every $i\in I$ we have
	\begin{align*}
		m_i\circ k_i\circ t&=m\circ t \\&= n\\&=m_i\circ h_i
	\end{align*}
	Since $m_i$ is mono we conclude that $k_i\circ t=h_i$.
	
	\smallskip \noindent
	$(2) \Rightarrow (1)$ The family $\{k_i\}_{i\in I}$ of coprojections witnesses that $[m]$ is a lower bound in $(\sub{M}{X}{X}, \leq)$ for $\{[m_i]\}_{i\in I}$. Let $[n]$ be another one, thus we have a cone $(n, \{h_i\}_{i\in I})$ in $\mathcal{M}/X$ and so a unique $t:N\to M$, where $N$ is the domain of $n$, such that $k_i\circ t=h_i$. Thus
	\begin{align*}
		m\circ t &= m_i\circ k_i\circ t\\&=m_i\circ h_i\\&=n
	\end{align*}
	We can thus conclude that $[n]\leq [m]$.
\end{proof}
\begin{corollary}
Let $\mathcal{M}$ be a class of monos in a category $\X$, then the following are true:
\begin{enumerate}
	\item if $\mathcal{M}$ contains $\id{X}$ then $(\sub{M}{X}{X}, \leq)$ has a maximum;
	\item if $\mathcal{M}$ is stable under pullback, closed under composition and $\X$ has pullbacks of arrows in $\mathcal{M}$ along arrows in $\mathcal{M}$ then $(\sub{M}{X}{X}, \leq)$ has binary infima.
\end{enumerate}
\end{corollary}
...
\begin{proposition}\label{cor:pbpres}
	Let $\mathcal{M}$ be a class of monos in a category $\X$ stable under pullbacks.  Let $f\colon X\to Y$ be an arrow admitting a pullback along any arrow $m\colon M\to Y$. Then the pullback functor $f^*\colon \mathcal{M}/Y\to \mathcal{M}/X$ preserves all limits.
\end{proposition}
\begin{proof}
	Let $()$
...
We end this appendix showing how to construct a factorization system given a suitable class of monos $\mathcal{M}$.
