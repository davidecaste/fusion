...
rewriting~\cite[Section~4]{Ehrig78}\cite[Chapter 3, Sec. 3.4]{CorradiniMREHL97}.
...
\begin{definition}[Strong Enforcing rewriting systems]
  A left-linear DPO rewriting system is \emph{strong enforcing} if
...
the same derivations. This can hinder the very idea of using
...
\begin{example}
  ...
  $\der{D}'$ rule $\rho_1$ uses the node which was in the initial
  graph. From the technical point of view, the property of being
  switch equivalent is not closed by prefix, something which prevents
  to derive a sensible concurrent sematics: if we limit to the first
  two steps, we obatain derivations $\dder{D}_0 \cdot \dder{D}_1$ and
...
%
Moreover, we next show that a class of rewriting systems, comprising
...
\begin{definition}[Graph signature]
  ...
  we define the \emph{depth} of a vertex $v\in V$ in the following
  way:
  \[
    \dph(v)=\begin{cases}
...
\begin{definition}[Node-merging graphical rewriting systems]
  Let $\mathcal{G}=(V, E, s, t)$ be a directed acyclic graph, the
  ...
  DPO-rewriting system $(\gph{G}, \R)$ such that, for every rule
  ...
  \item for every $v\in V$ and $x\in L(v)$, there exists a vertex $w$
    ...
    $L(p)\colon L(w)\to L(v)$.
  \item $r\colon K\to R$ is mono on vertices of depth $0$: for every
    $v\in V$ such that $\dph(v)=0$, the component $r_v:K(v)\to R(v)$
    is mono.
  \end{enumerate}
\end{definition}

For instance, in this view, the category $\cat{Graph}$ can be obtained
...
Another interesting class of well-switching rewriting systems is given by the so-called E-graphs.
...
\begin{example}[E-graphs]
  ...
  in~\cite{BaldanGM06}, closely related to Egg
  graphs~\cite{WNW:egg}. Formally, $\cat{EGraphs}$ can be seen as the
...
When dealing with left-linear rules, such a characterisation fails. Leaving aside the formal details (the interested reader can refer to \Cref{def:permcon}), the intuition is quite simple. In the linear case one proves that two derivations is using the same rules in different orders are switch equivalent when the colimits of the two derivations, seen as diagrams, are isomorphic and the isomorphism properly commutes with the embeddings of the rules.
...
research. \todo{P: cite, forse questo? https://arxiv.org/abs/2208.06295} We plan to make the correspondence precise
