...
switch equivalence of derivations based on so-called processes or
consistent permutations~\cite{add citations}.\todo{Handboo, graph processes, tobias}
...
\begin{example}
  ...
  the mappings from the interface to the left- and right-hand
  sides. Rules $\rho_0$ and $\rho_1$ are linear: $\rho_0$ generates a
  new node, while $\rho_1$ deletes an edge. Instead, $\rho_2$ is non
...
When dealing with left-linear rules, such a characterisation fails. Leaving aside the formal details (the interested reader can consult Appendix~\ref{})\todo{P: possiamo mettere in appendice la def. consistent permutation? e forse anche l'implicazione che vale per i left linear?}, the intuition is quite simple. In the linear case one proves that two derivations is using the same rules in different orders are switch equivalent when the colimits of the two derivations, seen as diagrams, are isomorphic and the isomorphism properly commutes with the embeddings of the rules.
...
The class of well-switchabke adhesive rewting systems is vast. Generalising a result in~\cite{baldan2011adhesivity} we can show that one of its defining properties (the fact that sequential independence implies switchability) holds for a class of adhesive categories which include all presheaves over set. Moreover, the second property, uniqueness of switching, is ensured when rewriting possibly hierarchical graphical structures, as long as elements of the top sorts are not merged (e.g., one can consider rewriting systems over directed graphs where only nodes are merged, as it happens in most approaches to modelling calculi, biological systems and also in string diagrams).\todo{Giusto, referneces?}
...
In $\mathcal{M}$-adhesive categories the pushout complement is unique up to iso, if it exists. 
% amend the second defect.\todo{A: }
...
\begin{proof}
  ...
  and $m$, thus, \Cref{lem:pocomp} yields an isomorphism
...
>>>>>>> refs/remotes/origin/main
    ...
    \[\xymatrix{G_{\der{F},1} & \ar@{>->}[l]_{f_{\der{F},1}} D_{\der{F},1} \ar[r]^{g_{\der{F},1}}& G_2 \\L_0 \ar[d]^{m_{\der{E},1}} \ar[u]_{m_{\der{F},1}}& K_{1} \ar[d]^{k_{\der{E},1}} \ar[u]_{k_{\der{F},1}}\ar[l]_{l_1} \ar[r]^{r_1} & R_{1}  \ar[d]_{g_1\circ i_0}  \ar[u]^{g_1\circ i_0}\\ \ar@/^.5cm/[uu]^{\psi_1} G_{\der{E},1} & \ar@{>->}[l]^{f_{\der{E},1}} D_{\der{E},1} \ar@/^.5cm/[uu]^(.33){\phi_{1}}|\hole \ar[r]_{g_{\der{E},1}}& G_2 \ar@/_.5cm/[uu]_{\psi_2}}\]
...
\begin{proposition}
	...
	square below). Then there exist arrows
...
\begin{proof}
	\begin{enumerate}
		...
		\parbox{11cm}{The arrow $f$ is  the pushout of $l$ and so it is in $\mathcal{M}$. We can thus consider the $\mathcal{M}$-pushout square depicted on the right.
...
\begin{proof}   
	...
	$\xymatrix@C=15pt@R=15pt{&&R_0 \ar@/_.8cm/[ddrr]_(.2){i_0}|(.7)\hole
		...
		\ar[dl]|(.4)\hole_(.65){f_0}\ar[ur]|(.5)\hole^(.7){g_0}&&D_1
		\ar[dr]|(.4)\hole^(.65){g_1}
		\ar[ul]|(.5)\hole_(.65){f_1}&&R_1\ar@/^.6cm/[ddll]^(.2){j_1}|(.31)\hole|(.81)\hole\ar[dl]_{h_1}|(.6)\hole\\&G_0
		&&P_1\ar[dr]_{q_1}
		...
		\ar[ur]_{\hspace{-7pt}s_1\circ
			j_1}\ar@/^.8cm/[uurr]^(.2){j_1}&& L_0 \ar[ul]^{t_0\circ
			j_0\hspace{-7pt}} \ar@/_.8cm/[uull]_(.2){j_0} |(.69)\hole
	}	$
		\caption{Picture for the proof of \Cref{lem:indep-global-left}: switching of $\dder{D}_0$ and $\dder{D}_1$}
		\label{fi:first}
	\end{figure}
	...
	\begin{figure}[h]
		...
		$\xymatrix@C=15pt@R=15pt{&&R_1 \ar@/_.8cm/[ddrr]_(.2){a_0}|(.7)\hole
			...
			\ar[dr]|(.4)\hole^(.65){g_2}
			\ar[ul]|(.5)\hole_(.65){f_2}&&R_2\ar@/^.6cm/[ddll]^(.2){b_1}|(.31)\hole|(.81)\hole\ar[dl]_{h_2}\\&G_1
			&&P_2\ar[dr]_{d_1}
			...
			\ar[ur]_{\hspace{-7pt}x_2\circ
				b_1}\ar@/^.8cm/[uurr]^(.2){b_1}&& L_1 \ar[ul]^{y_1\circ
				b_0\hspace{-7pt}} \ar@/_.8cm/[uull]_(.2){b_0} |(.69)\hole
		}	$		
		\caption{Picture for the proof of \Cref{lem:indep-global-left}: switching of $\dder{D}_1$ and $\dder{D}_2$}
		\label{fi:second}
	\end{figure}
	...
	\begin{figure}
		...
		$\xymatrix@C=15pt@R=15pt{&&R_0 \ar@/_.8cm/[ddrr]_(.2){e_0}|(.7)\hole
			...
			\ar[dl]_{f_1\circ a_1\hspace{-6pt}}\\&K_0\ar[dr]^{k_0}\ar[dl]_{l_0}
		...
		\caption{Picture for the proof of \Cref{lem:indep-global-left}: switching of $\dder{D}_0$ and $S_{a_0,a_1}(\dder{D}_2)$}
		\label{fi:third}
	\end{figure}
	...
	$a$ given by $(e_0, e_1)$ provides us with:
	\[\xymatrix{}\]
	\begin{enumerate}
		...
		\begin{figure}
			\centering
			$\xymatrix@C=15pt@R=15pt{&&R_0 \ar@/_1cm/[ddrr]_(.35){i_0}|(.7)\hole
		...
		$(\alpha_0, \alpha_1)$.
		\iffalse \[\xymatrix{&&R_0 \ar@/_1cm/[ddrr]_(.35){\alpha_0}|(.7)\hole \ar[dr]^{g_1\circ i_0}&& L_2\ar@/^1cm/[ddll]^(.35){\alpha_1}  \ar[dl]_{m_2}\\
