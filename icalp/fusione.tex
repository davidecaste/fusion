...
\begin{abstract}
  ...
  %
  In fact, the standard characterisation of independence does not hold anymore,
  and this requires a substantial revision of the 
  theory for the linear case, since a number of basic properties,
  essential in the development of a theory of concurrency, were
  derived from such a description.
  %
  The paper introduce a novel characterisation of independence, for which we prove a Church-Rosser property,
  properly extending the class of formalisms 
  that can be modelled in the adhesive framework. 
\end{abstract}
...
intuitively, a step is required to only consume, preserve and generate entities. 
However, in some situations it is also necessary to ``merge'' parts of the state. 
This happens already in the context of string diagrams mentioned above, but also 
...
As an initial step in performing this exploration, we observe that for left-linear
rules a number of fundamental properties that play a role in the
corresponding theory of rewriting need to be deeply revised, or plainly do not hold.
...
A difficult task, since the most natural idea, put forward in~\cite{baldan2017domains}, 
...
%
In this work we focus on a class of rewriting systems, which we call
...
equivalences capturing e-graphs are also very tame. As a sanity
...
isomorphism between the colimit of the derivations, called
...
\begin{enumerate}
\item when switching a derivation consisting of several steps, the result only depends on the overall permutation, not on the order of the switches;
...
sufficient, hence it cannot be used for proving (1) and (2). However property (1) still holds. Instead only a weak form of (2) can be proved. This is intimately related to the possibility of expressing disnjuctive forms of causality.
...
\begin{example}
  ...
  parallel edges is ?\todo{P: su simple graphs riusciamo a dire qualcosa? Non sono reg-adesivi, giusto?}, i.e.
...
\begin{definition}[Sequential independence]
  ...
  \emph{sequentially independent} and write $\dder{D}_0 \seqind \dder{D}_1$.\todo{Eliminata la nozione di weakly
...
\begin{definition}[Switch equivalence]
  ...
  $\der{D} \shift{\transp{i}} \der{E}$. Then \emph{switch
    equivalence} is the least equivalence on derivation such that if
  $\der{D} \shift{\transp{i}} \der{E}$ then
  $\der{D} \equiv^s \der{E}$.
\end{definition}
...
\begin{definition}[Good independence pair]
  ...
  $u_1\colon K_1\to P$ into the pullback.\todo{P: tolto ``given by \Cref{prop:tec}'', penso non sia bene avere un riferimento all'appendice qui}
  % given by \Cref{prop:tec} .
  %
  We will say that $(i_0, i_1)$ is a \emph{good independence pair} if
  the first two squares depicted  
  below are pushouts and if the pushout of $r_1 : K_1 \to R_1$ and $u_1 : K_1 \to P$ exists\todo{P: forse si possono togliere i quadrati sotto e dire a parole?}
  \[
    \xymatrix{
...
A left-linear DPO rewriting system $(\X, \R)$ is \emph{very tame} if it is tame and, for every derivation $\der{D}:=\{\dder{D}_{i}\}_{i=0}^1$, there is at most one independence paire between $\dder{D}_0$ and $\dder{D}_1$.
\end{definition}

The previous definition might sound restrictive. However, it is immediate to see that linear rewriting systems are all very tame (see Proposition~\ref{pr:weak}).
%
...
\begin{restatable}{lemma}{lemIde}\label{lem:ide}
	Let $\der{D}$ and $\der{D}'$ be two derivations in a linear DPO-rewriting system $(\X, \R)$ with the same length $l$. If $\id{[0,l]}:[0,l-1]\to [0,l-1]$ is a consistent permutation, then $\der{D}$ and $\der{D}'$ are abstraction equivalent.  
\end{restatable}
[Proof in \Cref{lemide-proof}]

\begin{example}
	\todo{questo non è vero in generale}
\end{example}
...
\begin{enumerate}
	...
	\item there exists a consistent permutation $\sigma\colon [0,\lgh(D)-1]\to [0,\lgh(D)-1]$ between them.
\end{enumerate}
\end{restatable}
...
%
%
%In particular, the two properties allows one to obtain a \emph{canonical form} for switching sequences: whenever two derivation sequences are switch equivalence via some permutation $\sigma$, then each one can be obtained from the other by applying switchings to pairs of rule applications which $\sigma$ reverses (the so-called inversions).\todo{P: questa prima parte da riportare in parte in intro}
%
%The failure of the characterisation of switch equivalence via
%consistent permutations for left-linear rules leads to the
%question asking whether the properties above hold.

We argued in the introduction that the characterisation of switch equivalence based on consistent permutations 
is the key to derive some simple but fundamental properties for linear rewriting systems, namely
\emph{confluence of switching} and  \emph{globality of independence}.
In this section we show that actually much of the theory can be retained: confluence of switching still holds, 
while globality of independence holds in a weak form, conceptually linked to the fact that fusions introduce a form of disjunctive causality. 
Finally, a canonical form for switching sequences can also be recovered.

We start with two lemmas dealing with derivations of length
$3$. Despite being technical, these lemma are the fundamental building blocks for proving properties (1) and (2) above.
...
\begin{restatable}[confluence of switchings]{lemma}{lemSwitchConfluence}
  ...
  that $(i_0,i_1)$ is a good independence pair between $\dder{D}_0$
  and $\dder{D}_1$, $(a_0,a_1)$ one between $\dder{D}_1$ and
  $\dder{D}_2$ and $(e_0, e_1)$ one between $\dder{D}_0$ and
  $S_{a_0,a_1}(\dder{D}_2)$.\todo{P: non e' meglio qui limitarci a very tame, cosi' da evitare riferimenti a good pairs?}
\end{restatable}
...
independent. \todo{P: da dire meglio e Ci vorrebbe un esempio}
...
\todo{P: anche qui sopra mi limiterei a very tame, cos\i' da evitare riferimenti a good e se si riesce anche alle indepdencen pairs}
  %
...
We first need some notation on permutations. A \emph{permutation} on
$\interval{n}$ is a bijection
$\sigma : \interval{n} \to \interval{n}$. We call it a
...
$\transp{i}[j]$.
% Whenever $j = i+1$ we call it an \emph{adjacent transposition}.
...
\land\ \perm(i) > \perm(j) \}$.
%
Clearly, $\perm$ is the identity iff $\inv{\perm}=0$.
...
We also need some notation on switching sequences. Consider derivation sequences $\der{D}_i$ for $i \in \interval[0]{n}$
related by switches
\begin{center}
  $\der{D}_0 \shift{\nu_1} \der{D}_1 \shift{\nu_2} \ldots \shift{\nu_n}
\der{D}_n$
\end{center}
It will be referred as a \emph{switching sequence}.  Given $i \leq j$,
we denote by $\nu_{i,j}$ the composition
$\nu_j \circ \nu_{i-1} \circ \ldots \nu_i$. We will say that the
switching sequence consists of inversions if for all
$i \in \interval[0]{n}$ the transposition $\nu_i$ is an inversion for
$\nu_{i,n}$.\todo{P: probabilmente questi e forse la notazione sulle
  permutazioni vanno messi quando si parla di switching, ma ora non
  ho la forza di ragionare}
...
\begin{corollary}
Let $(\mathcal{X}, \mathcal{M})$ be a pair of type $\mathcal{B}^+$, then every left-linear DPO-rewriting system is tame.
\end{corollary}
...
\label{lemide-proof}
\begin{proof}
	If $\der{D}$ and $\der{D}'$ are empty then the thesis follows at once since an abstraction equivalence is just an isomorphism. Let now $\der{D}$ and $\der{D}'$ be, respectively $\{\dder{D}_{i}\}_{i=0}^n$ and $\{\dder{D}'_i\}_{i=0}^n$. We construct by induction on $i\in [0,n]$ an abstraction equivalence such that, for every $i$, $\iota'_{G'_{i+1}}\circ \phi_{G_{i+1}}=\xi_{\id{[0,n]}}\circ \iota_{G_{i+1}}$.
...
We need to recall some notion from the theory of permutations.
\begin{definition}
Let $\sigma:[0,n]\to [0,n]$ be a permutation, an \emph{inversion} for $\sigma$ is a pair $(i,j)$ such that $i<j$ and $\sigma(j)< \sigma(i)$. 
\end{definition}
...
\begin{proof}$(1\Rightarrow 2)$ This follows from \Cref{prop:coswitch}.
