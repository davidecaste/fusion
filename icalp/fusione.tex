...
%  potentially in parallel.
  When can two consecutive steps performed by a computing device be
...
captures when 
%sequential 
two steps are causally unrelated and thus could be executed in any 
order, and possibly in parallel. 
%
Seminal contributions to this line of research have been
...
switch equivalence of sequences of steps based on what are 
called processes or consistent permutations~\cite{Handbook,tobias}.
%\todo{Handbook, graph processes, tobias}
...
\begin{proof}   
	...
	We can now define $\beta_0\colon R_0\to Q_3$ as $d_1\circ \beta'_1$ and $\beta_1\colon L_1\to Q_5$ be $n_1\circ \beta'_1$, so that
		...
		\begin{figure}[h]
			...
			\caption{Picture for the proof of \Cref{lem:indep-global-left}: switching of $S_{i_0, i_2}(\dder{D}_0)$ and $\dder{D}_2$}
			\label{fi:fourth}
		\end{figure}
