...
%
Two of the seminal contribution to this line of research have been
Mazurkiewicz's traces~\cite{xxx} and Winskel's event structures~\cite{xxx}
%
...
nowadays considered the standard one for these structures, mostly thanks to its 
flexibility: the rules allow to specify that, 
in a rewriting step, some entities in the current state are removed, some others are 
needed for the rewriting step to occur but remain unaltered and some new entities 
can be generated. Concerning concurrency, the chosen notion is shift equivalence,
and independence is formally captured by the so-called Church-Rosser 
theorem of DPO rewriting~\cite{CMREHL:AAGT}.
...
\rem{Qui l'idea \`e mostrare che valgono per $\mathcal{M}$ le naturali generalizzazioni delle propriet\`a delle adesive. Credo si possa dirle in prosa e metterle in appendice.}{%
...
\rem{Questo penso tutto in appendice, eventualmente tenendo la decomposition o richiamandola quando si parla di switch}{%
...
\end{theoremEnd}
\begin{proof}\begin{proofEnd}
		Both pairs $(k, f)$ and $(k', f')$ are pushout complements of $l$ and $n$, thus, \Cref{lem:pocomp} yields an isomorphism $\phi_D\colon D\to D'$. Computing we have
		\[
...
\begin{definition}[Sequential independence]
  ...
  % \emph{weakly sequentially independent} if an independence pair exists. If such  independence pair is unique we will say that $\dder{D}$ and $\dder{D'}$ are
  \emph{sequentially independent}.\todo{Eliminata la
    nozione di weakly indepedendent, giusto?}
\end{definition}
...
\end{theoremEnd}
\begin{proof}\begin{proofEnd}
	The first point of \Cref{def:switch} is obviously satisfied. The second one follows at once since 
...
\begin{proof}
	\begin{proofEnd}
		Consider the arrow $u_1\colon K_1\to P$ obtained using \Cref{prop:tec}. It fits in the diagram:
		...
		Applying \Cref{lem:radj} to the pair of squares below now gives us the wanted $q_0\colon P\to D_{\der{E},0}$.
...
\rem{Questa e' utile solo per definire i filler, giusto?. Sposterei in appendice o integrerei nella def. di filler}
{
...
\begin{theoremEnd}[category=sec3]{proposition}\label{prop:tec}
	Let $(i_0, i_1)$ be an independence pair between two direct derivations $\dder{D}_0$ and $\dder{D}_1$ in a left-linear DPO rewriting system $(\X,\R)$. Consider the pullback of $f_1\colon D_1\mto G_1$ along $g_0\colon D_0\to G_1$ (first square below),  then there exist the arrows $u_1\colon K_1\to P$ and $u_0\colon K_0\to P$ fitting in the central and right square, Moreover, the right one is always a pullback.
...
By \Cref{prop:pbpoad,lem:pb1} we get that the left half of the second rectangle is a pullback. 
	\end{proofEnd} \qedhere 
\end{proof}
}
...
Let also the right square above be a pullback.Let $u_0\colon K_0\to P$ and $u_1\colon K_1\to P$ be the arrows of \Cref{prop:tec}, we will say that $(i_0, i_1)$ is a \emph{good independence pair} if the first and second square below are pushouts and if the third square exists and it is a pushout too.
...
\rem{Questa e' la prova che il filler implica lo switch, giusto? In appendice}{
\begin{remark}\label{rem:deco} Let $(i_0, i_1)$ be a good independence pair between the direct derivations $\dder{D}_0$ and $\dder{D}_1$. We can then build the solid part of the diagram below.
	...
	\noindent
	\parbox{5.5cm}{\hspace{15pt}We can prove some other properties of the arrows appearing in the diagram above. The four rectangles aside are pushouts and their left halves are pushouts too. Therefore, by \Cref{lem:po1}, also their right halves are pushouts. 	Moreover, that $q_0$ and $a_0$ are pushouts of, respectively $l_0$ and $p_0$, thus they  are elements of $\mathcal{M}$. By \Cref{prop:pbpoad} the left halves of the first and third rectangles are pullbacks too.} 
	\parbox{3.5cm}{
...
\begin{remark}
  ...
  Moreover, by \Cref{prop:pbpoad}, the first square below is a pullback, therefore, by \cref{rem:deco}  the last three squares witness that $(j_0, j_1)$ is a good independence pair too.
...
\rem{Questo e' quanto gia' asserito sopra, mettere in appendice e riferire}{A source of tame left-linear DPO rewriting systems is given by the linear ones, as shown by the following proposition.
...
Since $(\X, \R)$ is linear, then $r_1\colon K_1\to R_1$ belongs to $\mathcal{M}$, thus it admits a pushout along $u_1\colon K_1\to P$, as desired. Moreover, let us consider the two rectangles below.
	...
	By hypothesis $r_0$ and $l_1$ are in $\mathcal{M}$, thus $f_1$ and $g_0$ belong to it too. The first point of \Cref{lem:popb}  yields the thesis. 
	\end{proofEnd}
\end{proof}
}
...
As a beginning step in our analysis of switch equivalence, we will establish three lemmas  dealing with derivations of length $3$. \todo{We will probably limit tho this}
...
We will start proving some general results regarding $\mathcal{A}$-Van Kampen and $\mathcal{A}$-stable squares, which, wll be used in the proof of the results of \Cref{sec:ade}. Let us begin recalling some classical results about  about pullbacks and pushouts
