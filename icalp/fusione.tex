...
% additional text
\iffull
\newcommand{\full}[1]{{\color{blue}#1 full}}
%\newcommand{\short}[1]{}
\else 
\newcommand{\full}[1]{}
%\newcommand{\short}[1]{#1}
...
\begin{abstract}
  ...
  of two steps means that they could be executed on any order, and
  ...
  % 
  An apparently minimal, yet technically hard enhancement, which
  allows to properly extend the formalisms that can be modelled in the
  adhesive framework, and for which we prove a Church-Rosser property.
  % 
  % We refine the established notion of sequential independence
  % between steps, and ...
  The canonical notion of independence used for linear rewriting
  systems does not work out of the box, but it requires the
  restriction to suitable well-behaved classes of left-linear
  rewriting systems.  Additionally, a fundamental characterisation of
  the notion of independence fails when moving to left-linear
  rewriting systems. This requires a substantial revision of the
  theory as in the linear case, a number of basic properties,
  essential in the development of the theory of concurrency, were
  derived from such a characterisation.
 \end{abstract}
...
observe that moving to left-linear rules leads to the failure of
...
In words, we let $\der{D} \equiv^s \der{E}$ when one derivation can be obtained from the other via a sequence of switches.
