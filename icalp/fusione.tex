...
%
More to the point, for linear rewriting systems sequential independence of 
two sequences of steps can be characterised in terms of the existence of 
what is called a consistent permutation among them, which can be obtained through 
an isomorphism between their colimits~\cite{xxx}. This result, which is folklore for 
various categories of graphs, is explicitly proved here for $\mathcal{M}$-adhesive categories. 
%
...
Unfortunately, we show that for left-linear rewriting systems the characterisation of
...
%
Nevertheless, we will show that property (1) still holds, while only a weak form of (2) does.
...
First of all, we will provide a refined notion of independence, which is based 
in the concept of fillers. The issues was identified
in~\cite{baldan2011adhesivity}, where a subclass of adhesive
categories was identified where independence as induced by a filler
coincides with sequential independence (as induced by an independence
pair). Here it is extended to $\mathcal{M}$-adhesive categories.
...
%
In this work we focus on what we call
\emph{strongly tame rewriting systems}, where sequential independence ensures existence and
...
Church-Rosser is fully recovered for very tame rewriting systems.
...
\begin{definition}[Switch equivalence]
 ...
 $\nu_{1,m}$.
  ...
  We will say that two derivations $\der{D}, \der{E}:G\Mapsto H$ are \emph{switch
    equivalent} if there is a switching sequence, possibly empty, $\{\der{D}_{k}\}_{i=0}^m$ such that
...
\begin{definition}[Good independence pair]
  ...
  $u_1\colon K_1\to P$ into the pullback (see \Cref{prop:tec} for details). 
...
\begin{corollary}
Let $(\X, \mathcal{M})$ be a pair of type $\mathbb{B}^+$, then every left-linear DPO-rewriting system is tame.
\end{corollary}
...
Let $(\X, \R)$ be a tame rewriting system and $(i_0, i_1)$ an independence pair between $\dder{D}_{0}$ and $\dder{D}_1$, let also $\der{E}=\{\dder{E}_i\}_{i=0}^1$  be a switch of them along $(i_0, i_1)$. Then the transposition $(1,2):[0,1]\to [0,1]$ is a consistent permutation.
\end{lemma}
\begin{proof}
...
\begin{lemma}
	Let $\der{D}$ and $\der{E}$ be two derivations in a linear rewriting system $(\X, \R)$. Suppose that a consistente permutation $\sigma:[0, \lgh(\der{D})-1]\to [0, \lgh(\der{D})-1]$ between them is given. If $i\in [0, \lgh(\der{D})-1]$ is an index such that $\transp{i}$ is an inversion for $\sigma$, then the direct derivations $\dder{D}_i$ and $\dder{D}_{i+1}$ are sequentially independent.
\end{lemma}
\begin{proof}
	contenuto...
\end{proof}
