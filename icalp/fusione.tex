...
We will write $m\colon X\mto Y$ to denote that an arrow $m\colon X\to Y$ belongs to $\mathcal{M}$.
...
\begin{definition}[Switch equivalence]
  ...
  $\dder{E}_i \cdot \dder{E}_{i+1}$ for some $i \in [0,n-1]$ \todo{A: ma manca ``and  $\dder{D}_j = \dder{E}_j$ for each $j \not \in \{i,i+1\}$''??} we write
  ...
  \der{D}_m\]
 In such case, we will\todo{A: io cambierei tutti i ``we will'' in ``we''.} often denote by $\nu_{k}$ the transposition $\transp{i_k}$. Given $k \leq h$,
...
\begin{example}[E-graphs]
  ...
  Finally, in applications, one is interested in rewriting systems in which the right half of a rule is always mono but not regular in general, allowing us to identify more nodes. These rewring systems are all well-switching.
  

  \todo[inline]{Noi prendiamo sistemi di riscrittura left-linear e mono a destra, c'è una refernza?}  
\end{example}
