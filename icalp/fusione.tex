...
\begin{definition}[Good independence pair]
  ...
  \[
    \xymatrix@C=18pt{L_0 \ar[d]_{m_0}&& K_0
      \ar[d]_{k_0}\ar@{>->}[ll]_{l_0} \ar[r]^{r_0} \ar@{->}@/^.20cm/[ddrr]|(.3)\hole|(.55)\hole_(.7){u_0} & R_0
      \ar@/^.25cm/[drrr]|(.3)\hole^(.2){i_0} \ar[dr]|(.3)\hole_{h_0}
      && L_1 \ar@/_.25cm/[dlll]_(.2){i_1} \ar[dl]|(.3)\hole^{m_1}& K_1
      \ar[d]^{k_1}\ar@{>->}[l]_{l_1} \ar[rr]^{r_1} \ar@{->}@/_.20cm/[ddll]|(.3)\hole|(.55)\hole^(.7){u_1} && R_1 \ar[d]^{h_1} \\
      %
      G_0 &&
  ...
  $u_1\colon K_1\to P$ into the pullback.
  %given by \Cref{prop:tec} .
  %
  We will say that $(i_0, i_1)$ is a \emph{good independence pair} if
  the resulting squares, depicted  
  below, are pushouts and the pushout of $r_1 : K_1 \to R_1$ and $u_1 : K_1 \to P$ exists.
  \[
    \xymatrix@R=5pt{
      %
...
The category $\cat{Graph}$ is tame by the result in ..., while
example~\ref{ex:diff1} shows an adhesive non-tame category.
...
\begin{example}[E-graphs]
  Graphs with equivalences on nodes. Formally $V \to$
\end{example}
