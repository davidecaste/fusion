...
of independence . \todo{P: Too technical? Forse qui e' piu'
  esplicativo un esempio} Whenever two steps are sequential
independent
\begin{itemize}
...
identifies clearly different computations.\todo{P: da dire meglio, ma e' un punto centrale}
A refined notion of
...
\begin{definition}Let $\mathcal{M}$ be a class of monos in a category $\X$, closed under composition, containing all isomorphisms and stable under pullbacks and pushouts. Suppose that a  diagram
	...
	\begin{itemize}
		\item the \emph{mixed decomposition} property if, whenever $k$ is a monomorphisms and the right half of the previous diagram is a pullback, then the left one is a pushout.
...
\begin{theorem}Let $\X$ be an $\mathcal{M}$-adhesive category satisfying the mixedand the $\mathcal{M}$-pushout decomposition properties. Then every left-linear DPO rewriting system on $\X$ is tame.
\end{theorem}
\begin{proof}\qedhere 
\end{proof}
