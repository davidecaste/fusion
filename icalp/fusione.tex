...
of switch equivalence based on consistent permutations fails,
% : the existence of a consistent permutation is only necessary but
% not sufficient
hence it cannot be used for proving (1) and (2).
%
This leads to asking whether the two properties above hold.
...
We then define well-switching systems,
...
proving properties for \emph{strong enforcing} rewriting systems, a class of systems where sequential independence is well-behaved\todo{P: altrimenti mai menzionata nell'intro}
...
%\end{theorem}
$\mathcal{M}$-adhesivity is well-behaved with respect to  the construction of slice and functor caregories \cite{mac2013categories}, as shown by the following theorems from~\cite{ehrig2006fundamentals,lack2005adhesive}.
...
\begin{example}
  ...
  parallel edges, is $\reg{(\cat{sGraphs})}$-adhesive~\cite{behr2022fundamentals} but not quasiadhesive.
\end{example}
...
\begin{definition}[Switch equivalence]
 ...
 $\nu_{1,m}$.
  ...
  We say that two derivations $\der{D}, \der{E}:G\Mapsto H$ are \emph{switch
    equivalent}, written $\der{D}\equiv^{sh} \der{E}$, if there is a switching sequence, possibly empty, $\{\der{D}_{k}\}_{k=0}^m$ such that
...
indispensable that typically in the literature
it is not even stated,
in the sense that in the literature switchability is not introduced
...
\begin{example}
  ...
  \parbox{10cm}{ \hspace{15pt} Note that the two direct derivations
    are sequential independent. Howver there is no switch since the
...
\begin{example}
  ...
  $R_1 \to G_2$ that map nodes accoording to their numbering.
...
Still, there are adhesive rewriting systems which are not strong enforcing, as shown by the following example, which is however somehow pathological.
...
\begin{example}
  ...
  $\dder{D}_0' \cdot \dder{D}_1'$ which are not switch equivalent while
...
systems where the uniqueness of the independence pair is always ensured.
...
edges cannot). We first introduce the signature which is essentially
an acyclic graph where nodes can be seen as sorts.
...
\begin{example}[E-graphs]
  ...
  morphisms which preserve the equivalence, as considered
  ...
  each node to the corresponding equivalence class. Notice that the inclusion functor into  $[E \rightrightarrows V \to Q, \cat{Set}]$ creates pullbacks and pushouts \cite{mac2013categories} so that they are computed componentwise.
  ...
  preserving them. This characterization of regular monos and the fact that pullbacks and pushouts are computed componenetwise allows us to prove quasiadhesivity of $\cat{EGraphs}$ at once. Moreover, one can deduce that every rewriting system $(\X, \R)$ which is left-linear with respect to $\reg(\cat{EGraphs})$ is strong enforcing: this is done exploiting again the inclusion functor into $[E \rightrightarrows V \to Q, \cat{Set}]$.
...
\begin{restatable}[Globality of independence]{lemma}{lemIndepGlobalLeft}
  ...
  Let $\X$ be an $\mathcal{M}$-adhesive category and	$(\X,\R)$ a well-switching DPO rewriting system.
  For every  derivation
  $\der{D}=\{\dder{D}_i\}_{i=0}^2$  of lenght $3$, the following hold true:
  \begin{enumerate}
  \item
...
The second lemma capture the essence of the consistency of
...
While in the case of linear rewriting systems inversions can be
...
for a general class of adhesive categories which include all presheaves over $\cat{Set}$. Moreover, the second property, uniqueness 
...
\begin{proof}
    ...
    \parbox{7cm}{\hspace{15pt}Now, by \Cref{prop:unique} we get $\phi_0\colon D_{\der{E},0}\to D_{\der{F},0}$ and $\psi_1 \colon G_{\der{E},1}\to G_{\der{F},1}$ such that the diagram on the right commutes. But then we have
    	\[
...
\begin{proposition}
	...
	rewriting system $(\X,\R)$. Consider the pullback of
...
We begin this section recalling the definition of \emph{consistent permutation} \todo{REFERENZE} for the interested reader.
...
\begin{proof}   
	Since $(\X, \R)$ is well-switching, then, there exist, unique, independence pair 
$(i_0,i_1)$ between $\dder{D}_0$ and $\dder{D}_1$,	$(a_0,a_1)$ between $\dder{D}_1$ and $\dder{D}_2$ and	$(e_0, e_1)$ one between $\dder{D}_0$ and
	...
	\begin{enumerate}
		...
		\item In addition to the three diagram above, we have a fourth one, depicted in \Cref{fi:fourth}.
		\begin{figure}[h]
			\centering
...
Finally, we switch the first two direct derivations.
	...
	Moreover, we can characterize the pair $(x_0, x_1)$. If we apply the first point of \Cref{lem:indep-global-left} to $\dder{D}$ we get an independence pair between the last two steps of the derivation above, which, since $(\X, \R)$ is well-switching, must coincide with $(x_0, x_1)$. Thus we have two diagrams:
	...
	As usual, this last switch brings identities $\check{m}_0=\bar{f}_1\circ u_1$ and $g_2\circ j_0=\bar{g}_2\circ u_0$. Applyng the second point of \Cref{lem:indep-global-left} to $\dder{D}_0\cdot S_{j_0, j_1}(\dder{D}_2)\cdot S_{j_0, j_1}(\dder{D}_1)$ and the fact that $(\X, \R)$ is well-switching we get the following diagrams:
