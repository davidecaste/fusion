...
Let $\der{D}$ and $\der{D}'$ be two derivations in a  a linear DPO-rewriting system, $(\X, \R)$, with $\X$ an $\mathcal{M}$-adhesive category. Then the following are equivalent:
\begin{enumerate}
...
\subsection{A Canonical Form for  Switch Equivalences}\label{subsec:cano}

In the case of linear rewriting systems, the characterisation of
switch equivalence based on consistent permutations is the key to
derive some simple but fundamental properties for the theory, namely
\begin{enumerate}
...
consistent permutations for left-linear rules naturally leads to the
question asking whether also the properties above cease to hold.

In this section we show that actually much of the theory can be retained: confluence of switching still holds, globality of independence holds in a weak form conceptually motivated by the fact that fusions introduce disjubctive causality. Finally a canonical form for switching sequences can also be recovered.
...
indepdendently from the order of the switchings.
...
The next technical lemma is at the core of globality of independence. It shows that if we have a derivation consisting of three
rewriting steps, say $r_0\, r_1\, r_2$ where $r_0$ and $r_1$ are independent, then if we switch $r_2$ to the first position, obtaining a derivation $r_2\, r_0\, r_1$, the steps $r_0$ and $r_1$ are still independent.
...
\subsubsection{Proofs for \Cref{subsec:cano}}
