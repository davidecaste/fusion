...
\tikzstyle{node}=[circle, draw=black, minimum size=1mm, inner sep=1.5pt]
...
%  potentially in parallel.
  ...
  they could be executed in any order, and potentially in parallel.
  % 
  ...
  %, a very general setting for rewriting.  
  More precisely, we focus on left-linear rules, which
  not only consume, preserve and generate entities, as linear rules
  do, but may also ``merge'' parts of the state.
  % 
...
\item \emph{Strong Enforcing rewriting systems}\\ An
  ...
  rewriting system where all independence pairs are strong is called
...
\item \emph{Well-Switching rewriting
    systems}\\ Strong Enforcing and existing independence pairs are unique (hence
...
dynamics of distributed systems: the graph serves to represent the
...
rewriting~\cite[Chapter 3, Section~3.4]{CorradiniMREHL97}. %cite[Section~4]{Ehrig78}
...
-- is to maintain an
equivalence %\todo{P: more precisely a congruence}
over the nodes.
...
theory have been put forward, %to the best of our knowledge 
...
% of rules and focussing as far as DPO in concerned on quasi-adhesivity, 
...
different ways. As we will observe, this prevents the development
of a sensible theory of concurrency since switches are used as a basis
...
interest for rewriting. In particular, we identify a vast class of
...
%for well-switching rewriting systems.
...
consistent permutations~\cite{CMR:GP,Handbook,BaldanCHKS06,heindel2009category}.
In turn this is the key
to derive some basic but fundamental properties for linear rewriting
systems, namely
...
of switch equivalence via consistent permutations fails,
% : the existence of a consistent permutation is only necessary but
% not sufficient
hence it cannot be used for proving (1) and (2).
%
This leads to asking if the two properties above hold.
We show that much of the theory can be retained: globality of independence holds in a
weaker form, conceptually linked to the fact that fusions introduce a
form of disjunctive causality, while consistency
of switching holds unchanged. Finally, we prove that a canonical form
...
offers some thoughts on possible directions for future work.
The appendixes collect additional data, recalling basic results on
adhesive categories (Appendix~\ref{app:ade}), introducing and 
proving properties for \emph{strong enforcing} rewriting systems, a class of systems where the local Church-Rosser Theorem holds
(Appendix~\ref{app:fill}), and giving the proofs of the main 
results of our work (Appendix~\ref{omitted}).
...
\begin{notation}
  ...
  %\item
   ...
   % arrow $X\to 1$ from an object $X$ will be denoted by
    ...
    %the unique arrow $0\to X$. %When $\X$ is $\Set$ and $1$ is a singleton, $\delta_x$ will denote the arrow $1\to X$ with value $x\in X$.

  \item $\mor(\X)$, $\mon(\X)$ and $\reg(\X)$ denote the class of
    all arrows, monos and regular monos of $\X$, respectively.

  \item Given an integer $n\in \mathbb{Z}$, $[0,n]$ denotes the
...
The key property that $\mathcal{M}$-adhesive categories enjoy is the so-called \emph{Van Kampen condition}~\cite{brown1997van,johnstone2007quasitoposes,lack2005adhesive}. We
will recall it and examine some of its consequences. First of all, we
need to introduce some terminology and facts regarding subclasses of
$\mor(\X)$.
...
\parbox{10cm}{
  \begin{definition}[Van Kampen property]
    Let $\X$ be a category and consider the diagrams aside.
 %   
%    \hspace{15pt}
    ...
    We call a $\mor(\X)$-Van Kampen ($\mor(\X)$-stable) square simply a \emph{Van Kampen} (\emph{stable}) square.
  \end{definition}}
...
%\end{theorem}
$\mathcal{M}$-adhesivity is well-behaved with respect to  the construction of slice and functor categories \cite{mac2013categories}, as shown by the following theorems from~\cite{ehrig2006fundamentals,lack2005adhesive}.
...
\begin{example}
  ...
  adhesive where $E \rightrightarrows {V}$ is the two objects category
  with two morphisms $s,t \colon{E} \to {V}$. In a similar way,
  various categories of hypergraphs can be shown to be adhesive, 
  such as term graphs and hierarchical graphs~\cite{CastelnovoGM24}. We can also notice that the category $\cat{sGraphs}$ of simple graphs, i.e. graphs without
  parallel edges, is $\reg{(\cat{sGraphs})}$-adhesive~\cite{BehrHK23} but not quasiadhesive.
\end{example}
...
perform abstract rewriting using what is called the Double-Pushout
 (DPO) approach. We will now recall the basic definitions and properties
...
\begin{definition}[\!\!\cite{habel2012mathcal,heindel2009category}]
  ...
  called $\mathcal{M}$-\emph{linear}\todo{P: giusto?}
  %
  \full{\emph{(consuming)}}
  %
  ...
  \parbox{10cm}{\hspace{15pt}Given two objects $G$ and $H$ and a rule
    ...
    \emph{back-match}.  We denote a direct derivation $\dder{D}$
...
\begin{definition}[Derivation]
  ...
  %
  We denote by $\lgh(\der{D})$ the \emph{length} of derivation $\der{D}$.
 %
...
\parbox{8cm}{
  \begin{definition}[abstraction equivalence]
    ...
    for every $i\in [0, \lgh(\der{D})]$. We say that $\der{D}$ are $\der{D}'$ are \emph{abstraction
...
\begin{example}\todo{identità consistente non implica astrazione}Notice that the \Cref{ex:empty} cannot, in general, be generalised to non-empty derivations. A counterexample is the following one.
\end{example}
...
\begin{notation} We  denote any switch of two switchable direct derivations $\dder{D}_0$ and $\dder{D}_1$ along $(i_0, i_1)$ by $S_{i_0, i_1}(\dder{D}_0, \dder{D}_1)$. Moreover, we use $S_{i_0,i_1}(\dder{D}_1)$ and  $S_{i_0,i_1}(\dder{D}_0)$ to denote its components, in such a way that \[S_{i_0, i_1}(\dder{D}_0, \dder{D}_1)=S_{i_0,i_1}(\dder{D}_1)\cdot S_{i_0,i_1}(\dder{D}_0)\]
\end{notation}
...
%when viewing  derivations as concurrent computations, 
it is canonical to identify
derivations that are equal ``up to switching'', i.e. that differs
only for the order of independent steps. The same notion can be given
...
\begin{definition}[Switch equivalence]
 ...
 $\nu_{1,m}$.
  ...
  We say that two derivations $\der{D}, \der{E}:G\Mapsto H$ are \emph{switch
    equivalent}, written $\der{D}\equiv^{sh} \der{E}$, if there exists a switching sequence, possibly empty, $\{\der{D}_{k}\}_{k=0}^m$ such that
...
\begin{example}
  ...
  Take the set $S:=\{a,b,c\}$ and endow it with the smallest order
  ...
  \noindent
  \parbox{10cm}{ \hspace{15pt} Note that the two direct derivations
    are sequential independent. However there is no switch since the
    rule applied by $\dder{D}_1$ cannot be applied to $c$. In fact,
    the unique morphism $a\to c$ is $(a,c)$, thus we get the diagram
    aside. But $b$ and $c$ do not have a supremum in the poset
    underlying $\X$, thus we do not get a direct derivation from $c$.}
  \parbox{3cm}{
...
\begin{definition}[strong independence pair]
  ...
  for $i \in \{0,1\}$ and the mediating arrows $u_0\colon K_0\to P$ and
...
%   left-linear rewriting system with $\X$ $\mathcal{M}$-adhesive.
%   \[
%     \xymatrix@C=15pt{L_0 \ar[d]_{m_0}&& K_0
%       \ar[d]_{k_0}\ar@{>->}[ll]_{l_0} \ar[r]^{r_0} & R_0
%       \ar@/^.35cm/[drrr]|(.3)\hole_(.4){i_0} \ar[dr]|(.3)\hole_{h_0}
...
Along the lines of \cite{baldan2011adhesivity} we can identify a large
class of adhesive categories such that all left-linear rewriting
systems over such categories are strong enforcing. The details are in
\Cref{app:fill}. Here it is important only to remark that such class
includes $\cat{Set}$ and it is closed by comma and functor category
constructions.  As such, it includes essentially all categories (e.g.,
presheaves over set) that are typically considered for modelling
purposes. Notably, it contains the category $\cat{Graph}$ of directed
graphs.

Still, there are adhesive rewriting systems that are not strong enforcing, as shown by the following example, which is however somehow pathological.
...
\begin{example}[Non-strong enforcing left-linear rewriting system]
  ...
  of an independence pair which is not strong, and thus of a
  non-strong enforcing left-linear adhesive rewriting system.%  Instead every
  % left-linear system in category $\cat{Graph}$ is strong enforcing, by
  % the results of Appendix~\ref{app:fill}, summarized
  % in~\Cref{ex:graph-type-bb} and~\Cref{cor:llr-is-strEnf}.
\end{example}
...
As a sanity check, note that a source of strong enforcing left-linear rewriting systems is given by the linear ones, as shown by the following proposition.
...
the fact that there can be more than one independence pair between
the same derivations. This hinders the very idea of using
...
\begin{example}
  ...
  that there are two distinct independence pair, as the left-hand side
  ...
  graph. From the technical point of view, the property of being
  switch equivalent is not closed by prefix, and this prevents
  to derive a sensible concurrent semantics: if we limit to the first
  two steps, we obtain derivations $\dder{D}_0 \cdot \dder{D}_1$ and
  $\dder{D}_0' \cdot \dder{D}_1'$ that are not switch equivalent, while
  $\der{D} = \dder{D}_0\cdot \dder{D}_1 \cdot \dder{D}_2$ and
  $\der{D}' = \dder{D}_0'\cdot \dder{D}_1' \cdot \dder{D}_2'$ would
  be.
\end{example} 
...
The previous definition might sound restrictive. However, it is immediate to see that linear rewriting systems are well-switching (see Proposition~\ref{pr:weak}).
%
Moreover, Lemma~\ref{bono} will show that a class of rewriting systems, comprising
all the ones used in modelling the applications to the encoding of process calculi or of bio and chemical systems mentioned in the introduction,
are actually well-switching.
%\todo{citazioni?}
%
Roughly speakings, these are rewriting systems over possibly
hierarchical graphical structures obtained as algebras of suitable
signatures where rules are constrained not to merge elements of top
level sorts in the hierarchy (for graphs, nodes can be merged while
edges cannot). We first introduce signatures that are essentially
acyclic graphs whose nodes can be seen as sorts.

\begin{definition}
  Let $\mathcal{G}=(V, E, s, t)$ be a finite directed acyclic graph where $s,t\colon E\rightrightarrows V$.
  We define the \emph{depth} of a vertex $v\in V$ as
  \[
    \dph(v)=\begin{cases}
...
\begin{definition}[Node-merging graphical rewriting systems]
  Let $\mathcal{G}=(V, E, s, t)$ be a finite directed acyclic graph
  and $\G$ the free category over $\mathcal{G}$. The
  category $\gph{G}$ of \emph{$\mathcal{G}$-graphs} is the category
  $\Set^{\G}$. A
  \emph{node-merging graphical rewriting systems} is a left-linear
  rewriting system $(\gph{G}, \R)$ such that, for every rule
  $(l,r)$ with $l\colon K\to L$ and $r\colon K\to R$ in $\R$, the
  following hold
  \begin{enumerate}
  \item for every $v\in V$ and $x\in L(v)$, there exists a vertex $w$
    with depth $0$ and an arrow $p\colon w\to v$ (i.e.~a path in
    $\mathcal{G}$) such that $x$ belongs to the image of
    $L(p)\colon L(w)\to L(v)$;
  \item $r\colon K\to R$ is mono on vertices of depth $0$, i.e. for every
    $v\in V$ such that $\dph(v)=0$, the component $r_v:K(v)\to R(v)$
    is mono.
  \end{enumerate}
\end{definition}

For instance, the category $\cat{Graph}$ can be obtained
by taking as $\mathcal{G}$ the graph $e \rightrightarrows v$. The sort
of edges $e$ has depth $0$ while $v$, the sort of nodes, has depth
$1$. Hence, condition $1$ asks that in the left-hand side of each rule
there are no isolated nodes, while condition $2$ asks that the
morphism $r: K \to R$ is mono on edges, i.e. it can only merge nodes.
...
\begin{example}[E-graphs]
  ...
  constrained to have the component $Q$ surjective. 
...
Left-linear rewriting systems on $\cat{EGraphs}$, i.e. in which the right half of a rule is mono but not regular in general, 
...
In the linear case, the existence of a consistent permutation it is enough to characterise switch equivalence. \todo{FORSE QUA METTEREI DEI POINTER A QUALCHE LAVORO PASSATO?}
...
\full{[Proof in \Cref{thmswitch-proof}]}
...
switch equivalence, i.e. the globality of independence, the
consistency of switching, and the existence of a canonical form for
equivalence proofs, can be derived, in the linear case, from a
characterisation of switch equivalence in terms of consistent
permutations or processes.

When dealing with left-linear rules, such a characterisation fails. Leaving aside the formal details (the interested reader can refer to \Cref{def:permcon}), the intuition is quite simple. In the linear case one proves that two derivations using the same rules in different orders are switch equivalent when the colimits of the two derivations, seen as diagrams, are isomorphic and the isomorphism properly commutes with the embeddings of the rules.

This result fails when considering left-linear
derivations. This is readily seen by considering derivations $\der{D}$
in Fig.~\ref{fi:derD} and $\der{D}'$ in Fig.~\ref{fi:derD1}. We
already observed that $\der{D}$ and $\der{D}'$ are not switch equivalent.
% 
Still, it is easy to see that they are related by a consistent
...
$3$. Despite being technical, these lemma are the fundamental building blocks for obtaining our results.
...
\begin{example}
  ...
  $\der{F}$ in Fig.~\ref{fi:derF}. Observe that rule $\rho_4$ needs a self-loop on node $12$ to be
  applied. This can be generated either
  by $\rho_3$ or by $\rho_1$ merging nodes $1$ and $2$, hence at least
  one of them must precede the application of $\rho_4$. Indeed, in $\der{E}$ it can be easily seen that the second and the third step applying $\rho_1$ and $\rho_4$ are independent. However, we could switch twice the application of $\rho_3$, bringing it in the last position, obtaining a derivation $\der{F}'$ as depicted in Fig.~\ref{fi:derF1}, where the applications of  $\rho_1$ and $\rho_4$ are no longer independent.
...
\begin{restatable}[Globality of independence]{lemma}{lemIndepGlobalLeft}
  ...
  $\der{D}=\{\dder{D}_i\}_{i=0}^2$ the following hold
  \begin{enumerate}
  \item
    \label{lem:indep-global-left:1} if $\dder{D}_0\seqind \dder{D}_1$ and there is a switching sequence
    \[\der{D}\shift{(1,2)} \der{D}'\shift{(0,1)} \der{D}''\]
    then the last two steps of $\der{D}''$ are switchable;
    
  \item
    \label{lem:indep-global-left:2}
    suppose that $\dder{D}_1 \seqind\dder{D}_2$ with independence pair $(\alpha_0, \alpha_1)$ and that, moreover,  $\dder{D}_0\seqind S_{\alpha_0, \alpha_1}(\dder{D}_2)$, if there is a switching sequence
    \[\der{D}\shift{(0,1)} \der{E}'\shift{(1,2)} \der{E}''\]
    then the first two steps of $\der{E}''$ are switchable.
  \end{enumerate}
\end{restatable}
  %
...
\full{[Proof~\ref{lemSwitchConfluence-proof}]}


The lemmas above open the way to the key result of this section establishing a canonical form for
switching derivations.

The first result shows that whenever equating two derivation sequences by performing a sequence of switching, we can limit ourselves to switchings inverting the order of steps that in the target derivation has to be reversed, i.e. we can limit ourselves to applying inversions.
...
rewriting systems due to the fact that, as already observed, globality of independence holds in a weak form. For instance, if we get back to Example~\ref{ex:non-global} and consider derivation $\der{F}$ in Fig.~\ref{fi:derF}, the last two steps applying rules $\rho_1$ and $\rho_4$ can be switched, thus leading to a derivation, call it $\der{F}''$,  applying $\rho_3, \rho_4, \rho_1$. Such $\der{F}$ is switch equivalent to $\der{F}'$ in Fig.~\ref{fi:derF1}, hence starting from $\der{F}''$ we can obtain $\der{F}'$ via a switching sequence, but we cannot start by switching $\rho_1$ and $\rho_4$, despite the fact that this is an inversion.

Still, we can identify a
canonical form for switching derivations: at each step one can apply a switch to the inversion with higher index. 
...
\begin{restatable}[Canonical form]{corollary}{coCanonical}
  ...
  \begin{center}
    ...
    \shift{\nu_n} \der{D}_n \equiv_a\der{D}'$
  \end{center}
  where for each $i \in \interval{n}$ the transposition $\nu_i  = \transp{k}$ is such that 
...
The class of well-switching adhesive rewriting systems is large. Generalising a result in~\cite{baldan2011adhesivity},
we showed that one of its defining properties (the fact that sequential independence implies switchability) holds 
for a general class of adhesive categories that include all presheaves over $\cat{Set}$. Moreover, the second property, uniqueness 
of switching, is ensured when rewriting possibly hierarchical graphical structures, as long as elements of the sorts 
of depth 0  are not merged (e.g., one can consider rewriting systems over directed graphs where only nodes are merged, as it 
...
research~\cite{abs-2208-06295}. \todo{F: altre citazioni?} 
...
Let $\X$ be a category, and consider the diagram  aside, then the following hold
	\begin{enumerate}
...
\parbox{11cm}{
	\begin{definition}[Pushout complement]
		Let $f\colon X\to Y$ and $g\colon Y\to Z$ be two composable arrows in a category $\X$. A \emph{pushout complement} for the pair $(f,g)$ is a pair $(h,k)$ with $h\colon X\to W$ and $k\colon W\to Z$ such that the square aside commutes and it is a pushout.
	\end{definition}} \quad
\parbox{4cm}{\vspace{.1cm}
...
The class of categories of type $\mathbb{B}$ is closed under the same constructions of \Cref{cor:slice,thm:functors}. This can be deduced at once from the fact that in such categories pullbacks and pushouts are computed component-wise.
...
derivations which is worth mentioning.\todo{A: va tolto weakly?}
...
\begin{remark}
	...
	\Cref{thm:church} is made by two switchable derivations that will
...
\begin{proposition}
	\label{pr:weak}
	Let $(\X, \R)$ be a linear rewriting system. Then it is well-switching.
\end{proposition}
\begin{proof}
...
\begin{remark}\label{rem:po}
	Notice that $\gph{G}$ is adhesive and that pushouts are computed component-wise, thus if $g\colon F\to G$ is the pushout of $r\colon K\to R$, then $g_v\colon F(v)\to G(v)$ is mono for every $v$ with depth $0$.
\end{remark}
...
\begin{proof}
	...
	which entails $i_{1,v}=j_{1,v}$. Let now $v$ be a vertex with depth greater than $0$, and take $x\in L_1(v)$. By definition, we have $p:w\to v$ and $y\in L_{1}(w)$  such that $\dph(w)=0$ and $L(p)(y)=x$. By naturality we have
...
Before proceeding further, it is worth to point out the following fact about inversions.  Take a permutation $\sigma\colon [0,1]\to [0,1]$ and define 
\[\tnv{\sigma}:=\{(i,j)\in \inv{\sigma}\mid j=i+1\}\]

We still can prove that $\sigma$ is the identity if and only if $\tnv{\sigma}=\empty$. Indeed, if $(i,j)$ is an inversion for$\sigma$, then we have two case:
\begin{itemize}
	\item for every $h,k\in[i,j-1]$, $(h,k)$ is not an inversion, thus $(j-1, j)$ is an inversion belonging to $\tnv{\sigma}$; 
	\item there exists an inversion $(h,k)$ made by elements of $[i,j-1]$: then we can repeat the same argument and conclude proceeding by induction on $k-h$.
\end{itemize}

Now, let $\sigma$ be a permutation different from the identity, recall that $E_{\sigma}$ is the set all all $i$ such that $\sigma(i)=i$, then we have:
\begin{itemize}
	\item if $E_{\sigma}=\emptyset$ then the minimum $i$ such that $(i-1,i)$ belongs to $\tnv{\sigma}$ is $\sigma^{-1}(0)$. On the one hand,  $\sigma(\sigma^{-1}(0)-1)$ cannot be $0$ and thus $(\sigma^{-1}(0)-1, \sigma^{-1}(0))$ is in $\tnv{\sigma}$. On the other hand  
	\item 
\end{itemize}


let $k_{\sigma}$ the minimum $i$ such that $(i, i+1)$
\end{remark}
\fi 
	...
	\begin{enumerate}
		...
		\[\xi_\sigma\circ \iota_{G_0}\circ m_{0}=\iota'_{G_0}\circ m'_{}\]
		 Therefore we have the solid part of the diagram below. \[\xymatrix@C=30pt{D_{j} \ar[ddrr]^{\iota_{D_j}}
		 	...
		 	and\Cref{cor:colim} yields $i_0\colon L_{t+1}\to$
		\item 
	\end{enumerate}
...
\begin{definition}[Consistent permutation]
	...
	a left-linear rewriting system.  Take two
	non-empty decorated derivations $(\der{D}, \alpha, \omega)$ and
	$(\der{D}', \alpha', \omega')$ with the same length and with
	isomorphic sources and targets.
...
$(i_0,i_1)$ between $\dder{D}_0$ and $\dder{D}_1$,	$(a_0,a_1)$ between $\dder{D}_1$ and $\dder{D}_2$ and	$(e_0, e_1)$ one between $\dder{D}_0$ and
	$S_{a_0,a_1}(\dder{D}_2)$. 	Using \Cref{rem:fill} we can build the diagrams in \Cref{fi:first,fi:second,fi:third}.  The first depicts the  switching $\dder{D}_0$ and $\dder{D}_1$ along  $(i_0, i_1)$, the second that of $\dder{D}_1$ and $\dder{D}_2$ along $a_0$ and $a_1$, while the third refers to $\dder{D}_0$ and
	...
	\begin{enumerate}
		...
		\item In addition to the three diagrams above, we have a fourth one, depicted in \Cref{fi:fourth}.
		\begin{figure}[h]
			\centering
...
\begin{proof}
  ...
  reorganise the first three switchings as
...
\end{figure}
  ...
  Lemma~\ref{lem:indep-global-left}(\ref{lem:indep-global-left:1})\todo{Check
    numbering} and we have that the corresponding steps in positions
  $i+1$ and $i+2$ in $\der{D}_2''$ are still sequential
  ...
  Fig.~\ref{fi:switchB2}, third column)
  \begin{center}
...
\begin{lemma}
  ...
  \begin{center}
    $\der{D}_0 \shift{\nu_0} \der{D}_1 \shift{\nu_1} \ldots
     \der{D}_n \shift{\nu_n} \der{D}_{n+1}$
  \end{center}
  AIf the subsequence starting from $\der{D}_1$ consists of
  inversions and $\nu_0=\transp{i}$ while
