...
Another construction that preserves
$\mathcal{M}$-adhesivity is the formation of the functor category.
...
As already mentioned, when viewing derivations as concurrent computations, 
for linear graph rewriting systems it is canonical to identify  derivations which are equal 
``up to switching''. The same notion can be given for left-linear rewriting systems relying on notion of switch.
...
not even stated explicitly, in the sense that switchability is not introduced
axiomatically as in Definition~\ref{def:switch}, but it is based on
the explicit construction of a switch.

When working with left-linear rewriting systems, sequential independence does not imply 
switchability (while the converse implication clearly holds), as shown by the example below.
...
\begin{example}
  ...
  of $Q$ is greater than every element of $P$, while elements of $P$
...
independence pairs are good for left-linear rewriting systems: ses
\Cref{app:fill}.
