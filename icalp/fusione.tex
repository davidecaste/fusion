...
Here we perform this exploration working at the level of $\mathcal{M}$-adhesive catgories, which encompass ...

As an initial step in performing this exploration, we observe that for
left-linear rules a number of fundamental properties that play a role
in the corresponding theory of rewriting need to be deeply revised, or
plainly do not hold.
%
...
witch equivalence of derivation sequences, identifying rewriting
sequences of steps which differs only for the order of independent
steps.

We single a well-behaved class of left-linear rewriting
...
On the one hand, we argue that they capture most categories of
interest for rewriting. In particular, we single out a class of
left-linear rewriting systems on graph-like structures which are very
well-switchable and we show that rewriting systems on graphs with
equivalences, similar in spirit to e-graphs are also
well-switchable. As a sanity check, also linear rewriting systems on
...
weakened form, results which hold in the linear case.  Church-Rosser
is fully recovered for well-switchable rewriting systems.


There is, however, a further, serious problem.  For linear
rewriting systems s, can be characterised in terms of the
existence of what is called a consistent permutation~\cite{xxx}
(roughly, an isomorphism between their colimits). This result, which
...
\item \emph{confluence of switching}: when transforming derivation
  sequences into equivalent ones by sqwitching independent steps, the
...
A source of strong enforcing left-linear DPO rewriting systems is given by the linear ones, as shown by the following proposition.
