...
%  potentially in parallel.
  ...
  % 
  We investigate the issue in the context of the Double Pushout (DPO)
  ...
  do, but may ``merge'' parts of the state.
  % 
...
is shift equivalence, and independence is formally captured by what is
...
interest for rewriting. In particular, we identify a class of
...
of switching still holds, while globality of independence  in a
weaker form only, conceptually linked to the fact that fusions introduce a
...
proving properties for \emph{strong enforcing} rewriting systems
...
by the \emph{Van Kampen condition}
...
\begin{example}
  ...
  %that map nodes consistently with their numbering.\todo{P: having names for the graphs would probably help}
  A switch of $\der{D}$ along such independence pair is the derivation
  $\der{E}$ in Fig.~\ref{fi:derE}.
...
\begin{definition}[Switch equivalence]
 ...
 $\nu_{1,m}$.
  ...
  We say that two derivations $\der{D}, \der{E}:G\Mapsto H$ are \emph{switch
    ...
    We denote this relation by $\der{D}\equiv^{sh} \der{E}$. 
\end{definition}
...
\begin{example}[Non-strong enforcing left-linear rewriting system]
  ...
  non-strong enforcing adhesive rewriting system.%  Instead every
...
\begin{example}
  ...
  $\dder{D}_0' \cdot \dder{D}_1'$ are not switch equivalent while
...
edges can not). We first introduce the signature which is essentially
an acyclic graph where nodes can be seen as sorts.

\begin{definition}[Graph signature]
  Let $\mathcal{G}=(V, E, s, t)$, with $s,t\colon E\rightrightarrows V$, be a finite directed acyclic graph,
...
\todo{cite something here? graph signatures?}
...
As in \cite{BaldanGM06}, one can study  left-linear rewriting systems  on $\cat{eGraphs}$ in which the right half of a rule is mono but not regular in general, allowing us to identify, but not merging, more nodes . These rewring systems are well-switching.
\end{example}
...
switch equivalence, i.e. the confluence of switching, the globality of
...
switch equivalence in terms of consistent permutations or
processes.
...
a different and sensibly more complex route for the proofs: confluence
...
\begin{example}
  ...
  one of them must precede the application of $\rho_4$. Indeed, in $\der{E}$ it can be easily seen that the second and the third step applying $\rho_1$ and $\rho_3$ are independent. However, we could switch twice the application of $\rho_3$, bringing it in the last position, obtaining a derivation $\der{F}'$ as depicted in Fig.~\ref{fi:derF1}, where the applications of  $\rho_1$ and $\rho_3$ are no longer independent.
...
For every  derivation
	...
	\begin{enumerate}
		...
		\[\der{D}\shift{(1,2)} \der{D}_1\shift{(0,1)} \der{D}_2\]
		then the last two steps of $\der{D}_2$ are switchable;
		...
		suppose that $\dder{D}_1 \seqind\dder{D}_2$ with independence pair $(\alpha_0, \alpha_1)$ and that, moreover,  $\dder{D}_0\seqind S_{\alpha_0, \alpha_1}(\dder{D}_2)$, if there is a switching sequence
			\[\der{D}\shift{(0,1)} \der{E}_1\shift{(1,2)} \der{E}_2\]
	then the first two steps of $\der{E}_2$ are switchable.
	\end{enumerate}
\end{restatable}
  %
...
\begin{restatable}[Confluence of switchings]{lemma}{lemSwitchConfluence}
	...
	\[\der{D} \shift{(0,1)} \der{D}_1 \shift{(1,2)} \der{D}_2 \shift{(0,1)}\der{D}_3\\ \qquad \der{D}\shift{(1,2)}\der{E}_1\shift{(0,1)} \der{E}_2 \shift{(1,2)}\der{E}_3 \]
Then $\der{D}_3$ and $\der{E}_3$ are abstraction equivalent. 
\end{restatable}
...
The first result shows that whenever equating two derivation sequences by performing a sequence of switching, we can limit ourselves to switchings inverting the order of steps that in the target derivation has to be reversed, i.e. we can limit ourselves to inversions.
