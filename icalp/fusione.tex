...
A derivation can now defined simply as a sequence of direct derivations. More precisely:

\begin{definition}[Derivation]
  Let $(\X, \R)$ be a left-linear DPO rewrityng system, with $\X$
  $\mathcal{M}$-adhesive. Given $G, H \in \X$, a \emph{derivation}
...
\begin{definition}[Switch]
  ...
  a derivation $\der{E}=\{\dder{E}_i\}_{i=0}^1$, using the same rules
  ...
  \[
    ...
    \xymatrix@C=1.1mm@R=8mm{
      ...
      & & {R_0} \ar[drr]_{h_0}  \ar@(r,ul)@{.>}^(.75){i_0}[drrrrrr]
      & & & & 
      % 
      {L_1}\ar[dll]^{m_1} \ar@(l,ur)@{.>}_(.75){i_1}[dllllll]
    ...
    \xymatrix@C=1.1mm@R=8mm{
      {L_1} \ar[d]_{m_1'}
      & & {K_1} \ar@{>->}[ll]_{l_1} \ar[rr]^{r_1} \ar[d]_{k_1'}
      & & {R_1} \ar[drr]_{h_1'}  \ar@(r,ul)@{.>}^(.75){i_1'}[drrrrrr]
      & & & & 
      % 
      {L_0}\ar[dll]^{m_0'} \ar@(l,ur)@{.>}_(.75){i_0'}[dllllll]
      & & {K_0} \ar@{>->}[ll]_{l_0} \ar[rr]^{r_0} \ar[d]^{k_0'}
      & & {R_0} \ar[d]^{h_0'} \\
      % 
      % 
      {G}
      & & {D_1'} \ar@{>->}[ll]^{f_1'} \ar[rrrr]_{g_1'}
      & & & & {H'} & &
      & &  {D_0'} \ar@{>->}[llll]^{f_0'} \ar[rr]_{g_0'}
      & & {H'}  }
    %
  \]
  such that there is an independence pair $(i_1', i_0')$ between
  $\dder{E}_0$ and $\dder{E}_1$ and 
  \begin{center}   
    $m_0=f_0' \circ i_0'$
    \qquad $h_1=g_1' \circ i_1'$
...
As already mentioned, when viewing derivations as concurrent computation, for linear graph rewriting systems it is canonical to identify  derivations which are equal ``up to switching''. The same notion can be given for left-linear rewriting systems relying on notion of switch.
...
\subsection{Relating sequential indepedence and switchability}


The fact that sequential indepedence implies switchability always
holds for linear rules. [REF] The result is so fundamental that it is
not even stated, in the sense that switchability is not introduced
...
\full{
  \begin{remark}
    ...
    independence pair $(i_1',i_0')$ between the component of a switch is
    uniquely determined.
  \end{remark}
}
...
\begin{example}
  ...
  $\der{D}_1$, $\der{D}_2$ each applying the corresponding rule
  $\rho_i$ is depicted in in Fig.~\ref{fi:derD}. The second and third
  steps $\der{D}_1$ and $\der{D}_2$ are clearly sequential
  independent, via an independence pairs which map nodes consistently
...
\begin{definition}[Good independence pair]
  ...
  \[
    \xymatrix@C=15pt{L_0 \ar[d]_{m_0}&& K_0
      \ar[d]_{k_0}\ar@{>->}[ll]_{l_0} \ar[r]^{r_0} \ar@/^.20cm/@{.>}[ddrr]|(.7){u_0} & R_0
      \ar@/^.35cm/[drrr]|(.3)\hole_(.4){i_0} \ar[dr]|(.3)\hole_{h_0}
      && L_1 \ar@/_.35cm/[dlll]^(.4){i_1} \ar[dl]|(.3)\hole^{m_1}& K_1
      \ar[d]^{k_1}\ar@{>->}[l]_{l_1} \ar[rr]^{r_1} \ar@/_.20cm/@{.>}[ddll]|{u_1} && R_1 \ar[d]^{h_1} \\
      %
      G_0 &&
      \ar@{>->}[ll]^{f_0} D_0 \ar[rr]_{g_0}&& G_1 &&
      \ar@{>->}[ll]^{f_1} D_1 \ar[rr]_{g_1}&& G_2\\
      %
      & & & & P \ar@{>.>}[ull]^{p_0} \ar@{.>}[urr]_{p_1}
    }
  \]

  Consider the pullback of $g_0$ and $f_1$, getting $p_i : P \to D_i$
  ($i \in \{1,2\}$) and the mediating arrows $u_0\colon K_0\to P$ and
  $u_1\colon K_1\to P$ (dotted part of the diagram).
  %
  We will say that $(i_0, i_1)$ is a \emph{good independence pair} if
  the first and second square below are pushouts and if the third
  square exists and it is a pushout too.
  \[
    \xymatrix{
      %
      K_0 \ar[r]^{r_0} \ar[d]_{u_0}& R_0 \ar[d]^{i_0} & K_1
      \ar@{>->}[r]^{l_1} \ar[d]_{u_1}& L_1 \ar[d]^{i_1}&K_1
      \ar[r]^{r_1} \ar[d]_{u_1}& R_1\ar@{.>}[d]^{j_0} \\
      %
      P \ar[r]_{p_1} & D_1 &P \ar[r]_{p_0} & D_0&P \ar@{.>}[r]_{q_1} &
      Q_1}
  \]
\end{definition}
...
\begin{restatable}[Three steps Lemma]{lemma}{lemIndepGlobalLeft}
  ...
  Let $(\X,\R)$ be a left-linear DPO rewriting system with $\X$ an
  $\mathcal{M}$-adhesive category. Consider a derivation
