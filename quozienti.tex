...
\newcommand{\cmd}{\mathbf{IC}}

\newcommand{\cori}{\mathbf{CR}}
...
\begin{definition}
	Let $\X$ be a category, a subcategory $\Y$ of $\X$ is \emph{coreflective} if the inclusion functor $I_\Y\colon \Y\to \X$ has a right adjoint $R_\Y$. A \emph{coreflective pair} $(\Y, R_\Y)$ is a pair made by a coreflective subcategory $\Y$ of $\X$ and a chosen right adjoint to $I_\Y$. A morphism between two coreflective pairs $(\Y, R_\Y)$ and $(\Z, R_\Z)$ is a functor $F\colon \Y \to \Z$ making the following diagrams commutative.
	\[\xymatrix{\Y  \ar[dr]_{I_\Y}  \ar[rr]^{F} &&\Z  \ar[dl]^{I_\Z}&&\X \ar[dr]^{R_\Z} \ar[dl]_{R_\Y} \\ & \X && \Y \ar[rr]_{F} && \Z }\]
...
Associated to a coreflective subcategory $\Y$ of $\X$ we have a class of arrows which enjoys some desiderable properties.
...
In general, the class $\mathcal{R}_{(\Y, R_\Y)}$ is not closed under pushout. Nonetheless such property is desirable and often enjoyed in the setting of DPO-rewriting. We are thus lead to the following definition.
