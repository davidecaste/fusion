...
{Department of Computer Science, University of Pisa, Italy}
 ...
 {MUR PRIN 20228KXFN2 ``STENDHAL'' and U. Pisa PRA 2022\_99 ``FM4HD''}
...
\begin{figure}
  ...
  \caption{A sequence of rewriting steps in .}
...
--- new version (according to latest mail, rules not yet up to date) ---

Just to get some basic intuition about left-linear rewriting systems, their capa
bilities and the problems which can arise, assume we want to model a situation i
n which a coffee comes with a bag of white sugar and a bag of brown sugar. The s
ituation is represented by the graph $G_0$ in Fig.~\ref{fi:coffee-rewriting}: th
e loop $\mathit{c}$ is the coffee, the two loops $b$ and $w$  represent the whit
e and brown sugar. The corresponding nodes are connected to the coffee node by a
n $s$-labelled edge to indicate that they can be possibly added to the coffee. T
he available rules are in Fig.~\ref{fi:coffee-rules}. Rules $\rho_w$ and $\rho_b
$ model the addition of white and brown sugar, respectively, to the coffee.
%
...
Once some sugar has been added, the coffee is ready to drink. This is represented by rule $\rho_r$ which check for the presence of sugar, represented by loop $s$, in the coffee, and bring it to the $cr$ state, coffee ready to drink.


A possible rewriting sequence is depicted in Fig.~\ref{fi:coffee-rewriting}, applying $\rho_w$, $\rho_b$ and $\rho_r$ in sequence, thus producing a coffee with both white and brown sugar, ready for drinking. A first interesting observation is that after applying rule $\rho_w$ to $G_0$, thus producing graph $G_1$, we can still apply rule $\rho_b$, with a non-injective match. Intuitively, once the coffee is ready to be drink because some sugar has been added, adding more sugar will not change its state. Indeed, the rules could be also be applied also in reverse order. Should they be considered independent? Moreover, since rule $\rho_r$ just require the presence of sugar, it could be anticipated, and applied after $r_w$. This might lead to think that $\rho_b$ and $\rho_r$ are independent. However, this is not completely clear, since in absence of $\rho_w$, the application of $\rho_b$ would be essential to enable $\rho_r$.
...
\begin{example}[Non-strong enforcing left-linear rewriting system]
  ...
  left-linear rewriting system in a $\mathcal{M}$-adhesive category which is quite
...
\begin{example}
  ...
  $\lambda_2$. Indeed, in $\der{F}$ it can be easily seen that the second
...
The first result shows that when equating two derivation sequences by performing a series of switchings, we can limit ourselves to switchings that inverte the order of steps that in the target derivation has to be reversed, i.e.~we can limit ourselves to applying inversions.
...
by switching $\lambda_0$ and $\lambda_2$, despite the fact that this is an inversion.
...
independence, identifying consecutive rewriting steps which are
independent and thus can be performed in reverse order is typically
closely related to that of parallel
independence~\cite{ehrig2014adhesive}. Parallel independence
characterises the independence of two rewriting steps starting from
the same object, and guarantees that the two steps can be applied in
...
get two coinitial parallel independent steps.  Under mild additional
conditions, one can have a notion of parallel rule in a way that
parallel independent steps can be also applied in parallel.
%
Even if this is not discussed explicitly in the paper, our theory can be easily accomodated to ecompass a notion of parallel independence and parallel rule application, enjoying the properties outlined above. To ensure this, however, one must require that the underlying category $\X$ has binary coproducts and that the class $\mathcal{M}$ is closed under them.
