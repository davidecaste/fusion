...
also necessary to ``merge'' parts of the state.  This happens
...
in biological/chemical processes~\cite{PUY:MBPE}. A similar mechanism
is at the core of e-graphs (equality graphs), used for
rewrite-driven compiler optimisations~\cite{WNW:egg}: rather than
merging nodes corresponding to equal expressions, the idea 
-- conceptually and, as we will see, also mathematically similar
-- is to maintain an
...
theory have been put forward~\cite{Ehrig1976,EHP:BRfTToHLRS,baldan2011adhesivity}, %to the best of our knowledge 
...
% Just to get some basic intuition about left-linear rewriting systems, their capabilities and the problems which can arise, assume we want to model a situation in which a coffee comes with a bag of sugar, a portion of milk and bag of cocoa. The situation is represented by the graph $G_0$ in Fig.~\ref{fi:coffee-rewriting}: the loop $\mathit{cf}$ is the coffee and the three edges $m$, $s$ and $c$  represent the milk, sugar and cocoa, respectively. Adding one of these elements, call it $x$, in the coffee is modeled as merging the corresponding node with the coffee node so that a self-loop $x$ is created.

% The available rules are in Fig.~\ref{fi:coffee-rules}. Rules $\rho_m$ models the addition of milk and sugar, while $\rho_c$ adds cocoa and sugar to the coffee. Once the sugar has been added, the coffee can be stirred and then it is ready to drink. This is represented by rule $\rho_r$ which check for the presence of sugar in the coffee, and stirs it.


% A possible rewriting sequence is depicted in Fig.~\ref{fi:coffee-rewriting}, applying $\rho_m$, $\rho_c$ and $\rho_r$ in sequence, thus producing a coffee with milk, sugar and cocoa, ready for drinking. A first interesting observation is that after applying rule $\rho_m$ to $G_0$, thus producing graph $G_1$, we can still apply rule $\rho_c$, with a non-injective match. Intuitively, the two rules do not interfere. Rather, $\rho_c$ applied after $\rho_m$ will have no effect on the sugar edge (intuitively, because sugar has been already added). Indeed, the rules could be also be applied in reverse order. Should they be considered independent? Moreover, since rule $\rho_r$ just require the presence of sugar, it could be anticipated, and applied after $r_m$. This might lead to think that $\rho_c$ and $\rho_m$ are independent. However, this is not completely clear, since in absence of $\rho_m$, the application of $\rho_c$ would be essential to enable $\rho_r$. ``There is great chaos under heaven, [hence] the situation is excellent!''
...
%
Once some sugar has been added, the coffee is ready for drinking. 
This is represented by rule $\rho_r$, which checks for the presence of sugar, represented by the loop $s$, in the coffee, 
and brings it to the $r$ state, ready for drinking.
...
% Here we perform this exploration at the
level of $\mathcal{M}$-adhesive categories,
which encompass a large family of DPO-based formalisms.
% adopted by researchers and  practitioners alike. 
...
interest for rewriting. Indeed, we identify general classes of
left-linear rewriting systems on graph-like structures that are
well-switching.
% and we show that rewriting systems on graphs with
% equivalences are so.
As a sanity check, we prove that linear rewriting systems 
%on $\mathcal{M}$-adhesive categories 
are well-switching.

On the other hand, we argue that a concurrency theory of rewriting can
be developed for well-switching rewriting systems, recovering, 
sometimes in
weakened form, most of the results that hold in the linear case.
% Specifically, the Church-Rosser theorem is fully recovered.
%
The development faces a main technical obstacle. In the
...
introducing the standard notion of independence and 
a novel axiomatic notion of switchability, showing that the
former fails to imply the latter for left-linear rewriting systems.
We then define well-switching rewriting systems,
a subclass of left-linear rewriting systems,
and we prove that the classical results for linear rewriting systems
are recovered for such a class.
Finally, in \S\ref{conclusions} we outline directions for future work.
%
%The appendices
% collect additional data,
Appendix~\ref{app:ade} recalls basic results on adhesive categories and
Appendix~\ref{app:fill}
presents \emph{strong enforcing} rewriting systems, a class of systems
where the local Church-Rosser Theorem holds. Appendix C of the
extended version provides the proofs
of our results.
...
notationally between $\X$ and its class of objects, so
``$X\in \X$'' means that $X$ is an object of $\X$.
We let $\mor(\X)$, $\mon(\X)$ and $\reg(\X)$ denote the class of
...
less than or equal to $n$; in particular, $[0,n]=\emptyset$ if
$n<0$.
...
\parbox{4.2cm}{\vspace{-1.35em}$\xymatrix@C=40pt{G'_i&D'_i \ar[r]^{g'_i}
...
%It can be seen that 
$\mathcal{M}$-adhesivity ensures the
uniqueness of the result of applying a rule: two
direct derivations using the same match are abstraction equivalent
(see Proposition~C.1 of the extended version).
...
\begin{definition}[Sequential independence]
  ...
  \[\xymatrix@R=16pt@C=15pt{L_0 \ar[d]_{m_0}&& K_0
      ...
      \ar[rr]_{g_1}&& G_2}
  \]
 In this case $(i_0,i_1)$ is called an  \emph{independence pair}.
    \end{definition}
...
equivalence (see Proposition~C.3 of the extended version).
...
$\sigma(k) = k$ otherwise; it is denoted as $\transp{i}[j]$.
%Such a transposition is
%% normally
%denoted as
%$\transp{i}[j]$ and we use the letter $\nu$ for transpositions.
%% 
Given a permutation $\perm$, % on $\interval[0]{n}$
an \emph{inversion}
...
holds for linear rules (see Proposition~C.8 of the extended version). The result is so
...
\begin{definition}[strong independence pair]
  ...
  into the pullback object (see Proposition~C.4 of the extended version for details). 
...
%  pathological since $\mathcal{M}=\mathsf{I(\X)}$, but it needs to be so, since natural examples are all in the well-behaved class mentioned above. 
 % .\todo{P: Scritto qualcosa, probabilmente da migliorare \\ D: per me va bene così}
  % Instead every
...
linear rewriting systems are well-switching (see Proposition~C.10 of the extended version).
%
...
%\todo{D: quest'ultima frase ha senso?}
%We first introduce signatures that are
...
Left-linear rewriting systems %(with respect to~$\reg(\cat{EGraphs})$) 
with respect to $\reg(\cat{EGraphs})$ are well-switching. They have been used in~\cite{BaldanGM06} for the graphical implementation of nominal calculi, where,
differently from~\cite{Gad07}, as a result of name passing the received name is not merged with a local one, % at the receiver,
but put in the same equivalence class, better tracing the causal dependencies among reductions.
\end{example}
...
refer to Definition~C.12 of the extended version), the intuition is quite simple. In the
...
trivially satisfied.  
% OLD VERSION
% This is readily seen by considering derivations $\der{D}$
% in Fig.~\ref{fi:derD} and $\der{D}'$ in Fig.~\ref{fi:derD1}. We
% observed that $\der{D}$ and $\der{D}'$ are not switch equivalent.
% % 
% Still, it is easy to see that they are related by a consistent\todo{le definiamo solo nell'appendice}
...
\begin{example}
  ...
  %\todo{P: rev.2 dice ``In the top-left and
   % top-middle nodes of $\rho_3$, the labels "12" should be "1" and
   % "2".''. Non e' corretto, ma magari si capisce poco ...}  
...
\todo{P: Qui si potrebbe fare lo stesso cambiamento del lemma precedente, usando
  
    $\dder{D}_0\cdot \dder{D}_1 \cdot \dder{D}_2 \shift{(0,1)}
    \dder{D}_1' \cdot \dder{D}_0' \cdot \dder{D}_2 \shift{(1,2)}
    \dder{D}_1' \cdot \dder{D}_2' \cdot \dder{D}_0''\shift{(0,1)}
    \dder{D}_2'' \cdot \dder{D}_1'' \cdot \dder{D}_0''$

$\dder{D}_0\cdot \dder{D}_1 \cdot \dder{D}_2 \shift{(1,2)}
    \dder{D}_0 \cdot \dder{E}_2' \cdot \dder{E}_1' \shift{(0,1)}
    \dder{E}_2'' \cdot \dder{E}_0' \cdot \dder{E}_1' \shift{(1,2)}
    \dder{E}_2'' \cdot \dder{E}_1'' \cdot \dder{E}_0''
    $
    ma credo che il dover usare $E$ renda meno facile la lettura}    
...
%characterises the independence of 
relates two rewriting steps starting from
the same object, %and guarantees that the two steps 
...
parallel independent steps can be also %applied both 
combined in a single rewriting step. %in parallel.
%
...
\parbox{10.4cm}{
	\begin{definition}Let $\mathcal{M}$ be a class of monos in a category $\X$, closed under composition, containing all isomorphisms and stable under pullbacks and pushouts. Suppose that the  diagram aside is given and the whole rectangle is a pushout. We say that $\X$ satisfies
		\parbox{14cm}{\begin{itemize}
			...
			\end{itemize}}
	\end{definition}} \parbox{3cm}{\vspace{-1.8cm}
	$\xymatrix{X \ar[d]_{a} \ar[r]^{f}& \ar[r]^{g} Y \ar[d]^{b}& Z \ar[d]^{c}\\ A \ar[r]_{h}& B \ar[r]_{k}& C}$}\\
...
\end{theorem}
\begin{proof}
	Let $\der{D}=\{\dder{D}_i\}_{i=0}^1$ be the derivation made by two sequentially independent derivations
...
\begin{definition} 
	...
	\begin{enumerate}
		...
		\item $\X$ has strict initial objects and for every $X$ the unique arrow $?_X\colon 0\to X $ belongs to $\mathcal{M}$;
...
%with its class of monos, 
is of type $\mathbb{B}$. Similarly, the category $\textbf{Inj}$ of sets and injective functions is quasiadhesive and, with regular monos, of type $\mathbb{B}$. 
\end{example}
...
\begin{example}
  \label{ex:graph-type-bb}
	The pair $(\mathbf{Graph}, \mon(\mathbf{Graph}))$ of graphs and their monos is of type $\mathbb{B}^+$  but not of type $\mathbb{B}$. Indeed, not every monomorphism of graphs is a coproduct coprojection.
\end{example}
