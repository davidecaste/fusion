...
\begin{abstract}
  ...
  While a consolidated theory exist for so-called linear rules which
  can consume, preserve and generate entities, this paper focuses on
  left-linear rules which may also ``merge'' parts of the state.
  % 
...
% i.e. where the behaviour is dictated by a set of rules that have to be instantiated with the system at hand,
...
sequences of rewriting steps that differ only for the order of
independent steps.
...
equivalences are so.\todo{weaker statement?}
...
weakened form, results which hold in the linear case.
% Specifically, the Church-Rosser theorem is fully recovered.
%
...
In we \S\ref{sec:equi} present the key novelties of our proposal,
...
``$X\in \X$'' means that $X$ belongs to the class of objects of $\X$.
...
less than or equal to $n$, assuming that $[0,n]=\emptyset$ whenever
$n<0$.
...
% will recall it and examine some of its consequences.
In order to define it we need to introduce some terminology.
% and facts regarding subclasses of
% $\mor(\X)$.
%
%\begin{definition}
...
% So equipped, we can introduce the notion of \emph{$\mathcal{A}$-Van Kampen square}.
  ...
  \begin{definition}[Van Kampen property]
    Let $\X$ be a category and consider the diagrams\\
    %
    \parbox{10cm}{
      aside.
      % 
    ...
    We call a $\mor(\X)$-Van Kampen ($\mor(\X)$-stable) square
    % simply
    a \emph{Van Kampen} (\emph{stable}) square.
  \end{definition}
...
\begin{theorem}
  ...
  $\mathcal{M}/X:=\{m\in \mathcal{A}(\X/X) \mid m\in
  \mathcal{M}$. Similarly, $X/\X$ is $X/\mathcal{M}$-adhesive with
  $X/\mathcal{M}:=\{m\in \mathcal{A}(X/\X) \mid m\in \mathcal{M}$.
...
\begin{example}
  ...
  graphs and hierarchical graphs~\cite{CastelnovoGM24}. Note that the category $\cat{sGraphs}$ of simple graphs, i.e.
  graphs without parallel edges, is
...
(DPO) approach.
%We next recall the basic definitions and properties of this approach. 
...
It can be seen that $\mathcal{M}$-adhesivity of $\X$ guarantees the
...
\begin{definition}[Sequential independence]
  ...
  $\dder{D}_i$ using rule $\rho_i = (l_i,r_i)$.  We say taht $\dder{D}_0$ and
  $\dder{D}_1$ are
  \emph{sequentially independent}
  if there is pair of arrows
  $i_0\colon R_0\to D_1$ and $i_1\colon L_1\to D_0$ such that the
  following diagram commutes, referred to as an  \emph{independence
    pair}: 
...
\begin{example}
  ...
  left-linear but not linear, as it ``merges'' two nodes.\todo{P: se $\rho_0$ generasse un arco tra 1 e 2, poi le derivazioni $\der{D}$ e $\der{D}'$ che non vogliamo siano switch equivalent hanno grafi intermedi non isomorfi, non solo con isomorfismi non consistenti, che mi pare meglio. Da vedere.}
...
We first recall some %concepts and
...
\begin{figure}
  ...
  \begin{subfigure}{0.58\textwidth}
    \xymatrix@C=12pt{
      a \ar[d]_{m_0} &
      a \ar[d]_{k_0} \ar[l]_{\id{a}} \ar[rr]^{(a,0)} & &
      0 \ar@/^.35cm/[drrrr]|(.315)\hole^(.75){\id{0}}
      \ar[dr]|(.3)\hole_{\id{0}} & & 
      a \ar@/_.35cm/[dllll]_(.75){(a,c)}
      \ar[dl]|(.3)\hole^{(a,0)} & &
      a \ar[d]^{(a,0)}\ar@{>->}[ll]_{\id{a}} \ar[rr]^{(a,b)} &&
      b \ar[d]^{(b,0)}\\
      c &
      \ar@{>->}[l]^{\id{c}} c \ar[rrr]_{(c,0)}& & & 0 &&&
      \ar@{>->}[lll]^{\id{0}} 0 \ar[rr]_{\id{0}}&& 0
    }
    \label{fig:diff1:rew}
    \caption{Two rewriting steps}
  \end{subfigure}
  \begin{subfigure}{0.20\textwidth}
    \xymatrix@C=16pt{a \ar[d]_{(a,c)}& a
...
\begin{example}
  ...
  $P = \mathbb{Z} \cup \{a,b,c\}$ and $\sqsubseteq$ defined by
  $m \sqsubseteq n$ if $m \geq n$, $a \sqsubseteq x$ for all $x \in P$
  and $b, c \sqsubseteq n$ for all $n \in \mathbb{N}$.
  %
  Let $\X$ be the  
  category associated to such order, which by \Cref{rem:iso} is
  $\mathsf{Iso(\X)}$-adhesive. 
  %
  ...
  $\dder{D}_1$ cannot be applied to $c$. In fact, the unique morphism
  $a\to c$ is $(a,c)$, thus we get the diagram aside. But $b$ and $c$
  do not have a supremum in the poset underlying $\X$, thus the arrows
  $(a,b)\colon a\to b$ and $(a,c)\colon a\to c$ do not have a
...
graphs.\todo{P: un reviewer dice ``it is unfortunate that you did not find space for describing the class in any detail, but please at least indicate how it compare to the class studied in [6].''. Mi pare sia chiaro, ma ...}

Still, there are adhesive rewriting systems that are not strong enforcing, as shown by the following example, which is however somehow pathological (recall that we fix $\mathcal{M}=\mathsf{Iso(\X)}$).

\begin{example}[Non-strong enforcing left-linear rewriting system]
  \label{ex:diff2}
  In light of \Cref{pr:church}, \Cref{ex:diff1} provides an example
  of an independence pair which is not strong, and thus of a
  non-strong enforcing left-linear adhesive rewriting system.\todo{P: rev2 dice "I don't think that's quite right. The category in Ex. 3.7 was not adhesive, wasn't it? It was only iso-adhesive, which is a non-condition.". Mi pare chiaro, ma se viene in mente un modo di esserlo di piu' ...}
  % Instead every
...
Back consistency with the theory of linear rewriting systems is ensured by the fact that all linear rewriting systems are strong enforcing (see \cref{prop:equi}).
...
% The previous definition might sound restrictive.
It is immediate to see that linear rewriting systems are well-switching (see Proposition~\ref{pr:weak}).
...
of top level sorts in the hierarchy (e.g., for graphs, nodes can be
merged while edges cannot). We first introduce signatures that are
essentially acyclic graphs whose nodes can be seen as sorts.
...
\begin{definition}[Node-merging graphical rewriting systems]
  ...
  $\Set^{\G}$. A
  \emph{node-merging graphical rewriting system} is a left-linear
  rewriting system $(\gph{G}, \R)$ such that, for each rule
  $(l\colon K\to L, r\colon K\to R)$ in $\R$, the
  following hold
  \begin{enumerate}
...
\begin{example}
  ...
  Fig.~\ref{fi:rules}) and add the rules $\rho_3$ and $\rho_4$ in
  Fig.~\ref{fi:rules-cas}.\todo{P: rev.2 dice ``In the top-left and top-middle nodes of $\rho_3$, the labels "12" should be "1" and "2".''. Non e' corretto, ma magari si capisce poco ...}
  Consider the derivation $\der{F}$ in
  Fig.~\ref{fi:derF}. Note that rule $\rho_4$ needs a self-loop on
  node $12$ to be applied. This can be generated either by $\rho_3$ or
  by $\rho_1$ merging nodes $1$ and $2$, hence at least one of them
  must precede the application of $\rho_4$. Indeed, in $\der{E}$ it
  can be easily seen that the second and the third step applying
  $\rho_1$ and $\rho_4$ are independent. However, we could switch
  twice the application of $\rho_3$, bringing it in the last position,
  obtaining a derivation $\der{F}'$ as depicted in
  Fig.~\ref{fi:derF1}, where the applications of $\rho_1$ and $\rho_4$
...
\begin{restatable}[Globality of independence]{lemma}{lemIndepGlobalLeft}
  ...
  \item
    ...
    suppose that $\dder{D}_1 \seqind\dder{D}_2$ with independence pair
    $(\alpha_0, \alpha_1)$ and that, moreover,
    $\dder{D}_0\seqind S_{\alpha_0, \alpha_1}(\dder{D}_2)$, if there
    is a switching sequence
    $\der{D}\shift{(0,1)} \der{E}'\shift{(1,2)} \der{E}''$
    then the
    first two steps of $\der{E}''$ are switchable.
  \end{enumerate}
\end{restatable}
  %
