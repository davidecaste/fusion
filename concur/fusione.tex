...
\begin{example}
  ...
  %
  Let $\X$ be the  
  category associated with such order, which by \Cref{rem:iso} is
...
i.e.~the fact that there can be more than one independence pair between
...
\begin{example}
  ...
  to derive a sensible concurrent semantics: 
  In fact $\der{D} = \dder{D}_0\cdot \dder{D}_1 \cdot \dder{D}_2$ and
  $\der{D}' = \dder{D}_0'\cdot \dder{D}_1' \cdot \dder{D}_2'$ are switch 
  equivalent, while if we consider the first
  two steps, derivations $\dder{D}_0 \cdot \dder{D}_1$ and
  $\dder{D}_0' \cdot \dder{D}_1'$  are not switch equivalent.
  % if we limit to the first
...
\begin{example}
  ...
  by $\rho_2$ merging nodes $1$ and $2$, hence at least one of them
  must precede the application of $\rho_4$. Indeed, in $\der{F}$ it
  can be easily seen that the second and the third step applying
  $\rho_2$ and $\rho_4$ are independent. However, we could switch
  twice the application of $\rho_3$, bringing it in the last position,
  obtaining a derivation $\der{F}'$ as depicted in
  Fig.~\ref{fi:derF1}, where the applications of $\rho_2$ and $\rho_4$
...
rewriting systems due to the fact that, as already observed, globality of independence holds in a weak form. For instance, if we get back to Example~\ref{ex:non-global} and consider derivation $\der{F}$ in Fig.~\ref{fi:derF}, the last two steps applying rules $\rho_2$ and $\rho_4$ can be switched, thus leading to a derivation, call it $\der{F}''$,  applying $\rho_3, \rho_4, \rho_2$. Such $\der{F}$ is switch equivalent to $\der{F}'$ in Fig.~\ref{fi:derF1}, hence starting from $\der{F}''$ we can obtain $\der{F}'$ via a switching sequence, but we cannot start by switching $\rho_2$ and $\rho_4$, despite the fact that this is an inversion.
...
\begin{proof}
	...
	$\mathcal{M}$ too entailing $i_1=i'_1$, too.
\end{proof}
...
\begin{proof}
	...
	\[g_{0,v}\circ i_{1,v}=m_{1,v}=g_{0,v}\circ j_{1,v}\]
