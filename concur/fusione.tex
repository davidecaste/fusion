...
well-switching.
% and we show that rewriting systems on graphs with
% equivalences are so.
As a sanity check, we also prove that linear rewriting systems on
...
% So equipped, we can introduce the notion of \emph{$\mathcal{A}$-Van Kampen square}.
  ...
  \begin{definition}[Van Kampen property]
    ...
    A $\mor(\X)$-Van Kampen square will be called simply \emph{Van
      Kampen} and similarly, a $\mor(\X)$-stable square is called
    \emph{stable}.
  \end{definition}
...
enforcing. Such class includes $\cat{Set}$ and it is closed
by comma and functor category constructions.  As such, it includes
...
This is a natural generalisation from adhesive to $\mathcal{M}$-adhesive catgeories of a class studied in \cite{baldan2011adhesivity} (see \Cref{app:fill} for details).
...
\begin{example}[Non-strong enforcing left-linear rewriting system]
  ...
  an independence pair which is not strong. This gives an example of a
  left-linear rewriting system in a $\mathcal{M}$-adhesive category
  which is not strong enforcing. The example is somehow
  pathological since $\mathcal{M}=\mathsf{Iso(\X)}$, but it needs to be so, since natural examples are all in the well-beahved class mentioned above. 
  .\todo{P: Scritto qualcosa, probabilmente da migliorare}
  % Instead every
...
observed that $\der{D}$ and $\der{D}'$ are not switch equivalent.
% 
Still, it is easy to see that they are related by a consistent
permutation, which is the identity on productions. The colimit of
both derivations is a single node with a self loop, i.e.~the
terminal object in $\cat{Graph}$, hence all commutativity requirements
are trivially satisfied.
...
\begin{example}
  \label{ex:non-global}
  Consider a new rewriting system in $\cat{Graph}$ consisting of the
  rules in $\rho_0$, $\rho_1$ and $\rho_2$ in
  Fig.~\ref{fi:rules-cas}.\todo{P: rev.2 dice ``In the top-left and
    top-middle nodes of $\rho_3$, the labels "12" should be "1" and
    "2".''. Non e' corretto, ma magari si capisce poco ...}  Consider
  the derivation $\der{F}$ in Fig.~\ref{fi:derF}. Note that rule
  $\rho_2$ needs the black self-loop on node $12$ to be applied. This can be
  generated either by $\rho_0$ or by $\rho_1$ merging nodes $1$ and
  $2$, hence at least one of them must precede the application of
  $\rho_2$. Indeed, in $\der{F}$ it can be easily seen that the second
  and the third step applying $\rho_1$ and $\rho_2$ are
  independent. However, we could switch twice the application of
  $\rho_0$, bringing it in the last position, obtaining a derivation
  $\der{F}'$ as depicted in Fig.~\ref{fi:derF1}, where the
  applications of $\rho_1$ and $\rho_2$ are no longer independent.
...
rewriting systems due to the fact that, as already observed, globality of independence holds in a weak form. For instance, if we get back to Example~\ref{ex:non-global} and consider derivation $\der{F}$ in Fig.~\ref{fi:derF}, the last two steps applying rules $\rho_1$ and $\rho_2$ can be switched, thus leading to a derivation, call it $\der{F}''$,  applying $\rho_0, \rho_2, \rho_1$. Such $\der{F}''$ is switch equivalent to $\der{F}'$ in Fig.~\ref{fi:derF1}, hence starting from $\der{F}''$ we can obtain $\der{F}'$ via a switching sequence, but we cannot start by switching $\rho_0$ and $\rho_2$, despite the fact that this is an inversion.
