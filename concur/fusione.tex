...
The key property of $\mathcal{M}$-adhesive categories is the \emph{Van Kampen condition}~\cite{brown1997van,johnstone2007quasitoposes,lack2005adhesive}. % We
...
%\begin{definition}
  ...
  $\mor(\X)$ is called\todo{il diagramma confonde la composizione}
  \begin{itemize}
    \parbox{11cm}{\item
...
% So equipped, we can introduce the notion of \emph{$\mathcal{A}$-Van Kampen square}.
  ...
  \begin{definition}[Van Kampen property]
    Let $\X$ be a category and consider\todo{diagramS?} the diagrams\\
    %
    \parbox{10cm}{
      aside.
      % 
...
\begin{example}
  ...
  $P = \mathbb{Z} \cup \{a,b,c\}$ and $\sqsubseteq$ is defined by
  ...
  $a\to c$ is $(a,c)$, thus we get the diagram in Fig.~\ref{ex:diff1:no-step}. But $b$ and $c$
...
Clearly, %It is immediate to see that 
...
\begin{definition}
  ...
  We define the \emph{depth} of a vertex $v\in V$ as\todo{ma ci serve tutta la depth?}
  \[
    \dph(v)=\begin{cases}
...
\begin{definition}[Node-merging graphical rewriting systems]
  ...
  $\Set^{\G}$. A
  \emph{node-merging graphical rewriting system}\todo{altro nome? source/leaf preserving} is a left-linear
  rewriting system $(\gph{G}, \R)$ such that  the
  following hold for each rule
  $(l\colon K\to L, r\colon K\to R)$ in $\R$
  \begin{enumerate}
