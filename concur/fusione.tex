...
\begin{abstract}
  ...
  While a consolidated theory exists for so-called linear rules which
...
SqPO~\cite{CorradiniHHK06} or PBPO$^+$~\cite{OverbeekER23}) and 10to its
...
\begin{example}[Non-strong enforcing left-linear rewriting system]
  ...
  .\todo{P: Scritto qualcosa, probabilmente da migliorare \\ D: per me va bene così}
  % Instead every
...
merged while edges cannot). The idea here is to consider as graphs presheaves on categories in which there are object which plays the roles of \emph{roots}, i.e.~objects which are not codomains of any arrows besides the identity. \todo{D: quest'ultima frase ha senso?}%We first introduce signatures that are
%essentially acyclic graphs whose nodes can be seen as sorts.

\begin{definition}[Root-preserving graphical rewriting systems]
	Let $\X$ be a category, an object $X\in \X$ is a \emph{root} if the only arrow with codomain $X$ is $\id{X}$. The category The
	category $\gph{X}$ of \emph{$\X$-graphs} is the category
	$\Set^{\X}$. A
	\emph{root-preserving graphical rewriting system} is a left-linear
	rewriting system $(\gph{X}, \R)$ such that  the
...
\begin{definition}[root-preserving graphical rewriting systems]
  ...
  $\Set^{\G}$. A
  \emph{root-preserving graphical rewriting system}\todo{altro nome? source/leaf preserving} is a left-linear
...
by taking as $\X$ the category generated by $E \rightrightarrows V$. The sort
...
\begin{definition}[Consistent permutation]
	...
	non-empty derivations $\der{D}$ and
	$\der{D}'$ with the same length and with
	isomorphic sources and targets.
