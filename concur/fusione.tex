...
\begin{example}[Non-strong enforcing left-linear rewriting system]
  ...
  pathological since $\mathcal{M}=\mathsf{Iso(\X)}$, but this is expected, 
...
merged while edges cannot). The idea here is to consider as graphs 
those presheaves on categories in which there are object that plays 
the roles of \emph{roots}, i.e.~objects that are not the codomain of 
...
\begin{definition}[Root-preserving graphical rewriting systems]
	...
	$(l\colon K\to L, r\colon K\to R)$ in $\R$ hold
	\begin{enumerate}
...
\begin{example}
  ...
  rules in $\sigma_0$, $\sigma_1$, and $\sigma_2$ in
  ...
  $\sigma_2$ needs the black self-loop on node $12$ to be applied. This can be
  generated either by $\sigma_0$ or by $\sigma_1$ merging nodes $1$ and
  $2$, hence at least one of them must precede the application of
  $\sigma_2$. Indeed, in $\der{F}$ it can be easily seen that the second
  and the third step applying $\sigma_1$ and $\sigma_2$ are
  independent. However, we could switch twice the application of
  $\sigma_0$, bringing it in the last position, obtaining a derivation
  $\der{F}'$ as depicted in Fig.~\ref{fi:derF1}, where the
  applications of $\sigma_1$ and $\sigma_2$ are no longer independent.
...
the last two steps applying rules $\sigma_1$ and $\sigma_2$ can be switched, thus leading to a derivation, 
call it $\der{F}''$,  applying $\sigma_0, \sigma_2, \sigma_1$. Such $\der{F}''$ is switch equivalent to $\der{F}'$ in Fig.~\ref{fi:derF1}, 
hence starting from $\der{F}''$ we can obtain $\der{F}'$ via a switching sequence, but we cannot start 
by switching $\sigma_0$ and $\sigma_2$, despite the fact that this is an inversion.
