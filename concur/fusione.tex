...
\begin{example}[Non-strong enforcing left-linear rewriting system]
  ...
  pathological since $\mathcal{M}=\mathsf{Iso(\X)}$, but it needs to be so, since natural examples are all in the well-behaved class mentioned above. 
...
merged while edges cannot). The idea here is to consider graphical structures as 
presheaves on categories in which there are objects which play the role of 
\emph{roots}, i.e.~objects which are not codomains of any arrow besides 
the identity. \todo{D: quest'ultima frase ha senso? A: Ok. Ho cambiato ``as graphs'' con ``graphical structures''.}%We first introduce signatures that are
%essentially acyclic graphs whose nodes can be seen as sorts.

\begin{definition}[Root-preserving graphical rewriting systems]
	Let $\X$ be a category, an object $X\in \X$ is a \emph{root} if the only arrow with codomain $X$ is $\id{X}$. The
...
Left-linear rewriting systems on $\cat{EGraphs}$,\todo{A: sostituirei ``, i.e. in which'' con ``where''. Giusto?} i.e.~in which the right half of a rule is mono but not regular in general, 
